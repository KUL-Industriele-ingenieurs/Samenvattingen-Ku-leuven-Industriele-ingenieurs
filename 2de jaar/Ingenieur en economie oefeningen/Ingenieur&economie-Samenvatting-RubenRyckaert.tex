...
Een markt wordt gedefineerd met de 4 P's met nog 2 extra P's die de maatschappij tonen
\begin{itemize}
...
\begin{enumerate}
    ...
    Dit creert een \textbf{waarde} voor de klant, wat leidt tot \textbf{tevredenheid}.
    De tevredenheid kun je peilen met een tevredensheidanalyse.
...
Je moet kijken naar de de vraag (hoeveel mensen willen het product kopen)
...
Een \belangrijk{Foot-hill effect} Pioneers gaan in het begin tijdens de introductiefase
veel kopen omdat ze het nieuwste van het nieuwste willen hebben.
...
afhankelijk van de markt en het product. \text{Bijv. Stijl, Mode, Rage}
...
\begin{description}
    ...
    \item\concept{Marketingmix strategie} De prijs moet overeenkomsten met de andere P's van de marketingmix:
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Totale kosten:} $ \rightarrow TCK = VK + VARK $
    \end{itemize}
...
\begin{description}
    ...
    \item\concept{Competitie binnne je markt}
...
Een bedrijf neemt dan een stratege aan afhankelijk van de prijs en kwaliteit van het product:


\begin{minipage}
    {0.4\textwidth}
    \begin{itemize}
        \item \textbf{Hoge prijs, hoge kwaliteit:} Premium stratege
        \item \textbf{Hoge prijs, lage kwaliteit:} Overgeprijsd
        \item \textbf{Lage prijs, hoge kwaliteit:} Good value stratege
        \item \textbf{Lage prijs, lage kwaliteit:} Economie stratege
    \end{itemize}
\end{minipage}
\hfill
\begin{minipage}
...
\begin{itemize}
    ...
    \item \textbf{Internationaal prijszetting:} Prijzen worden aangepast voor verschillende landen op basis van economische omstandigheden, concurrentie,
    invoertarieven\dots
\end{itemize}
...
Hoe dit eruit ziet. Je koopt een fiets uit je portomanee. Je hebt vorige periode gedaan
maar dit was een investering voor de lange termijn. Een investering kun
je afschrijven over meerdere periodes. 
Je gaat dan bij een afschrijving per periode een \% aan de kant leggen in een andere pot maar dat 
geld is niet weg. Je hebt de fiets al betaald uit je portomanee.
...
\addplot[fill=blue!60] coordinates {(1,20) (2,20) (3,20) (4,20) (5,20)};
\end{axis}
\end{tikzpicture}
\end{minipage}
...
\addplot[fill=red!60] coordinates {(1,40) (2,30) (3,20) (4,13) (5,7)};
\end{axis}
\end{tikzpicture}
\end{minipage}
...
\addplot[fill=green!60] coordinates {(1,7) (2,13) (3,20) (4,30) (5,40)};
\end{axis}
\end{tikzpicture}
\end{minipage}

\vspace{0.5em}
\noindent\textbf{Reele waarde afschrijving:} Hoe gaat afschrijven afhankelijk van het gebruik \textls{Hoeveel heb je een vrachtwagen gebruikt?}\\
\begin{minipage}[t]{0.58\textwidth}
\vspace{0pt}
\begin{itemize}
...
\addplot[fill=orange!60] coordinates {(1,15) (2,30) (3,25) (4,18) (5,10)};
\end{axis}
\end{tikzpicture}
\end{minipage}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=1\textwidth]{AfschrijfLimiet.png}
...
\begin{itemize}
    ...
    \[ \text{Opportunity Cost} = F_B - F_A = 1389,15 - 1500 = -€110,85 \]
\end{itemize}
\end{oefenblok}
...
\begin{figure}[ht]
      \centering
        \includegraphics[width=0.35\textwidth]{vastekosten.png}
        \includegraphics[width=0.55\textwidth]{image3.png}
...
\textit{Ze geven methodes maar daarna gaan ze andere namen geven voor
dezelfde methodes. Idk maar hier zijn ze:}
\begin{description}
    ...
    Wat zijn de gebruikte grondstoffen, lonen. Prodcutie 
    en verkoopskosten haal je eruit. 
    De winst die je dan maakt is de \belangrijk{marge}
    \textit{Je hebt dus niet direct winst maar gewoon hoeveel er overblijft deze kosten.}
...
Wanneer je gaat kijken naar standaardprijzen 
van de grondstoffen dat je gebruikt (wat is het prijs van 1kg staal)
dan heb je \belangrijk{standaard kostprijsberekening}
...
\begin{description}
    ...
    Deze kostprijs berekeningen hangt af van het verleden
    en is dus niet altijd representatief voor de toekomst.
    Dingen zoals lonen, grondstoffen prijzen etc kunnen veranderen.
    ...
    \newline
    Historsiche kostprijzen tellen gewoon alle kosten op die gemaakt zijn.
    Je hebt dus geen inzicht in vaste en variable kosten.

    \begin{oefenblok}[title=Oefening: Wijnhandelaar - Toeslagen en Productmix]
        \textbf{Situatie:}
    ...
    \begin{oefenblok}[title=Oefening: Industriële Standaard Kostprijsberekening]
    ...
    \item\concept{Evenredige kostprijsberekening:} 
    De industriele kostprijsberekening is ook niet perfect.
    ...
    \newline
    Je gaat dus kijken naar de vaste en variable kosten
    en een \belangrijk{break-even analyse} doen.
...
Je wilt zien of je bereken nu wel klopt met de werkelijke kostprijs.
Na je periode ga je deze berekening doen.



\frm{Afwijkingsanalyse}{W = afwijking = K' - K = (p' - p) * q' + (q' - q) * p'}
{
Met W = afwijking in euro's
K' = werkelijke kostprijs
K = geplande kostprijs
p' = werkelijke prijs
p = geplande prijs
q' = werkelijke hoeveelheid q = geplande hoeveelheid
}


\begin{oefenblok}[title=Afwijkingsanalyse - Begroting vs Realisatie]
    \textbf{Situatie:}
...
\begin{oefenblok}[title=Oefening uit examen: Activity Based Costing (ABC)]
    \textbf{Situatie:}

    Het maken van product A en B kost 3.500 euro in totaal
    ...
    Het producten verbruiken de activiteiten als volgt:
    ...
    \begin{tabular}{lrr}
    \hline
    \textbf{Activiteit} & \textbf{Product A}\\
    \hline
    Machine-uren (tarief €13,333...) & 100 uur $\times$ €13,33 = & €1.333,33 \\
    Inspecties (tarief €25,00) & 10 $\times$ €25,00 = & €250,00 \\
    Verpakkingen (tarief €1,666...) & 200 $\times$ €1,67 = & €333,33 \\
    \hline
    \textbf{Totaal Product A} & & \textbf{€1.916,66} \\
    \hline
    \textbf{Activiteit} & \textbf{Product B}\\
    \hline
    Machine-uren (tarief €13,333...) & 50 uur $\times$ €13,33 = & €666,67 \\
    Inspecties (tarief €25,00) & 30 $\times$ €25,00 = & €750,00 \\
    Verpakkingen (tarief €1,666...) & 100 $\times$ €1,67 = & €166,67 \\
    \hline
    \textbf{Totaal Product B} & & \textbf{€1.583,34} \\
    \hline
    \multicolumn{2}{r}{\textbf{Controle (som):}} & \textbf{€3.500,00} \\
    \hline
    \end{tabular}
...
Bekijk de slides of het boek voor een voorbeeld extra voorbeelden van ABC.

\begin{examenbox}
    Op het examen moet je zoiezo oftwel een tradionele kostprijsberekening
...
\subsection{Types van financiele analyse:}
\begin{description}
...
\begin{description}
    ...
    \begin{itemize}
        ...
        \frm{Schuldratio}{Schuldratio = \frac{Totale schuld}{Totale activa} = \frac{Totale schuld}{Totale schuld + Eigen vermogen}}
        ...
        \frm{Schuld/eigenvermogen ratio}{Schuld/eigenvermogen ratio = \frac{Schuld}{Eigen vermogen}}
        {}
...
Als de liquiditeitsratio (Eigen vermogen / Vreemd vermogen) en de winstgevendheid (hoeveel winst je maakt)
hoog zijn, is het bedrijf fit.

Als je verlies maakt maar je hebt veel geld, is het bedrijf structuureel ziek.
...
\begin{oefenblok}[title=Lineair Programmeren — De Ambachtelijke Werkplaats]
...
\begin{oefenblok}[title=Investeringsanalyse — De Astrazenetech Case]
...
De Netto Huidige Waarde is \textbf{positiv} (€16.514). 
...
\begin{oefenblok}[title=Lineair Programmeren — Rylon Parfums]
...
\textbf{Verwerking naar Luxe (per dl):}
\begin{center}
\begin{tabular}{|l|c|c|}
\hline
...
\begin{oefenblok}[title=Productie Optimalisatie bij een Bedrijf]
...
\textbf{Standaard notatie:}
\begin{align*}
\text{Max } Z &= 3X_1 + 7X_2 + 5X_3 \\
\text{S.t. } X_1 + X_2 + X_3 &\leq 50 \\
2X_1 + 3X_2 + X_3 &\leq 100 \\
X_1 &\geq 0
\end{align*}

\textbf{Simplex notatie (met spelvariabelen):}
\begin{align*}
Z - 3X_1 - 7X_2 - 5X_3 &= 0 \\
X_1 + X_2 + X_3 + S_1 &= 50 \\
...
\textbf{Stap 2:} Bereken CRT (Column Ratio Test): $\min\left(\frac{50}{1}, \frac{100}{3}\right) = \min(50, 33.33) = 33.33$
...
\begin{itemize}
    \item Voor $S_1$: $\frac{50}{1} = 50$ (betekent: $S_1$ wordt 0 als we 50 eenheden $X_2$ produceren)
    \item Voor $S_2$: $\frac{100}{3} = 33.33$ (betekent: $S_2$ wordt 0 als we 33.33 eenheden $X_2$ produceren)
\end{itemize}
...
\begin{warningbox}[title=Interpretatie van Pivot-operatie]
...
\begin{itemize}
    ...
    \item We gebruiken daarbij alle beschikbare machinetijd op
...
\textbf{Rijoperaties:}
\begin{itemize}
    \item $R_2' = R_2 / 3$ (maak pivot element 1)
    \item $R_1' = R_1 + 7R_2'$ (elimineer $X_2$ in Z-rij)
    \item $R_3' = R_3 - R_2'$ (elimineer $X_2$ in $S_1$-rij)
\end{itemize}
...
\textbf{Stap 4:} Nieuwe pivot kolom: $X_3$ (negatieve waarde: $-\frac{8}{3}$)

\textit{Uitleg:} Er staat nog een negatieve waarde in de Z-rij, dus we zijn nog niet optimaal. We herhalen het proces.

\textbf{Stap 5:} Bereken CRT: $\min\left(\frac{50/3}{2/3}, \frac{100/3}{1/3}\right) = \min(25, 100) = 25$
...
\textbf{Rijoperaties:}
\begin{itemize}
    \item $R_3' = R_3 \times \frac{3}{2}$ (maak pivot element 1)
    \item $R_1' = R_1 + \frac{8}{3}R_3'$ (elimineer $X_3$ in Z-rij)
    \item $R_2' = R_2 - \frac{1}{3}R_3'$ (elimineer $X_3$ in $X_2$-rij)
\end{itemize}
...
\begin{warningbox}[title=Interpretatie van niet-basis variabelen]
...
\begin{warningbox}[title=Stappenplan Sensitiviteitsanalyse voor Basis-variabele]
...
\begin{itemize}
    \item De \textbf{3} in de Z-rij = huidige prijs van $X_1$ (uit oorspronkelijke doelfunctie)
...
\textbf{Rijoperaties:}
\begin{itemize}
    \item $R_2' = R_2 \times 2$ (maak pivot 1)
    \item $R_1' = R_1 + R_2'$
...
\textbf{Rijoperaties:}
\begin{itemize}
    \item $R_3' = R_3 \times \frac{2}{5}$ (maak pivot 1)
    \item $R_1' = R_1 + R_3'$
