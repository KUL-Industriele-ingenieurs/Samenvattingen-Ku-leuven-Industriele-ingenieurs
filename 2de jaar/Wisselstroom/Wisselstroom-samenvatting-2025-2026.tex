...
\newcommand{\stapreden}[1]{\par\vspace{1.4mm}\noindent{\small\color{schoolGray}#1}\par\vspace{-2mm}}
...
{Met $\underline{Y}$ de admittantie in siemens (S), $G$ de conductantie en $B$ de susceptantie in S.
    Let op: $G \neq 1/R$ zodra er ook reactantie is --- je moet de hele breuk $1/\underline{Z}$
    uitrekenen.}

Waarom dat handig is: bij \textbf{parallel}takken tellen admittanties gewoon op
($\underline{Y}_{tot} = \underline{Y}_1 + \underline{Y}_2 + \dots$), net zoals impedanties optellen
bij \emph{serie}takken. Bij drie of meer parallelle takken is dat een pak minder rekenwerk dan
telkens $\frac{\underline{Z}_1\underline{Z}_2}{\underline{Z}_1+\underline{Z}_2}$ herhalen.
...
uit dezelfde ene voorwaarde $\text{Im}\{\underline{Y}\} = 0$. Hier loont het om met de
\emph{admittantie} te werken, want de twee takken staan parallel.

\stapreden{De spoeltak, met noemer reëel gemaakt:}
\[\underline{Y}_L = \frac{1}{R_L + j\omega L}
...
\[\underline{Y} = \underbrace{\frac{R_L}{R_L^2 + (\omega L)^2}}_{\text{re\"eel}}
...
\stapreden{En de dynamische weerstand is wat er van $\underline{Y}$ overblijft, want het imaginaire deel is nu weg. Vul de gevonden voorwaarde $R_L^2 + (\omega_0 L)^2 = L/C$ meteen in:}
\[\underline{Y}(\omega_0) = \frac{R_L}{R_L^2 + (\omega_0 L)^2} = \frac{R_L}{L/C} = \frac{R_L C}{L}
    \qquad\Longrightarrow\qquad
    \boxed{R_p = \frac{1}{\underline{Y}(\omega_0)} = \frac{L}{C R_L}}\]
