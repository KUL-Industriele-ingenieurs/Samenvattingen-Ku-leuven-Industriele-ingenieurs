...
Dit vak heeft namelijk geen formularium op het examen
dus je moet deze allemaal goed kennen!

Ik raad altijd aan om examenoefeningen te maken die je kunt vinden op studforum \url{https://wiki.studforum.net/pub/bac2/elektromagnetisme}

\section{Hoofdstuk 1: Basis wisselstroom theorie}

Eerst een paar basis formules die je moet kennen:
...
\frm{$I(t)$}{I(t) = I_m \sin(\omega t)}{Met $I(t)$ de wisselstroom op tijdstip t, $I_m$ de piekstroom
    (maximale stroom) en $\omega$ de pulsatie in rad/s.}
...
\begin{examenbox}
    ...
    Weet dat imaginaire impedanties (zoals spoelen en condensatoren) 90 graden verschoven zijn t.o.v.\ reële impedanties (zoals weerstanden).
\end{examenbox}

Als je je fasors goed definieert, kun je geometrische formules gebruiken
...
\begin{examenbox}[EXAMENTIP: Gegarandeerde theoriepunten!]
    Dit hoofdstuk is \belangrijk{gegarandeerde examenstof}: van de vijf korte theorievragen gaat er
    vrijwel altijd minstens één over elektrische veiligheid (bv.\ de vijf gouden regels, de
    MCB-karakteristiek, de RCD-werking, overstroom vs.\ kortsluiting of aanraakspanning). Dit leren
    is letterlijk gratis punten pakken!

    Bovendien komt deze leerstof integraal terug in latere vakken van de opleiding, zoals
    \emph{OIS (Ontwerp van Industriële Systemen)} en \emph{Distributie van Elektrische Energie}.
    Wat je hier goed leert, heb je dus meteen al beet voor je latere modeltraject.
\end{examenbox}

De cursus deelt dit hoofdstuk op in vier logische pijlers:
\keyterm{gevaren} $\rightarrow$ \keyterm{veilig werken} $\rightarrow$ \keyterm{bescherming tegen overstroom} $\rightarrow$
\keyterm{bescherming tegen elektrische schokken}.
...
\begin{itemize}
    \item \keyterm{Rechtstreekse aanraking (Direct contact):} een persoon raakt een geleider aan die
          in normale toestand onder spanning staat (bv.\ blote fasedraad L1 of actieve klem).
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{assets/indirect_contact.png}
    \caption{Onrechtstreekse aanraking (indirect contact): door een isolatiefout vloeit een foutstroom
        $I_{\text{fault}}$ via de beschermingsgeleider PE en de aardverspreidingsweerstand. Dit veroorzaakt
        een foutspanning $U_z$ over het toestel, die door een snelle uitschakeling geneutraliseerd moet worden.}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/overcurrents_in_iz_ib.png}
        \caption[Relatie van de ontwerpstromen]{Relatie van de ontwerpstromen: $I_b \le I_n \le I_z$.}
        \label{fig:overcurrents}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/fuses_smeltzekeringen.png}
        \caption{Klassieke smeltveiligheden (fuses).}
        \label{fig:fuses}
    \end{minipage}
\end{figure}
\FloatBarrier

\textbf{Dimensioneringsregel voor leidingbeveiliging:}
Om een kabel tegen oververhitting te beschermen moet voldaan zijn aan:
\[I_b \le I_n \le I_z \qquad\text{en}\qquad I_2 \le 1{,}45 \cdot I_z\]
\begin{itemize}
    \item $I_b$: ontwerpstroom / bedrijfsstroom van de belasting (load current).
    \item $I_n$: nominale aanspreekstroom van het beveiligingstoestel (MCB/zekering).
    \item $I_z$: continu toelaatbare stroom door de kabel (permissible current, afhankelijk van doorsnede en plaatsingswijze).
    \item $I_2$: conventionele uitschakelstroom van de beveiliging binnen de conventionele tijd ($1{,}45\,I_n$ voor een MCB).
\end{itemize}
...
\begin{examenbox}[EXAMENVRAAG: RCD / Aardlekschakelaar]
    ...
    \textbf{Synoniemen:}
    \begin{itemize}
        \item Aardlekschakelaar
        \item Differentieelschakelaar / Verliesstroomschakelaar
        \item RCCB (\emph{Residual Current Circuit Breaker})
        \item RCD (\emph{Residual Current Device})
    \end{itemize}

    \textbf{Beschermingsdoel:}
    \begin{itemize}
        \item \belangrijk{Persoonsbeveiliging ($30\text{ mA}$):} bescherming tegen elektrocutie bij
              zowel directe als onrechtstreekse aanraking.
        \item \belangrijk{Brandbeveiliging ($300\text{ mA}$ of $500\text{ mA}$):} voorkomt dat sluimerende
              isolatiefouten in leidingen ontbranden door lokale warmteontwikkeling ($P = I_\Delta^2 R$).
    \end{itemize}
\end{examenbox}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.44\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/rcd_diff_foto.png}
        \caption{4-polige aardlekschakelaar ($40\text{ A}$, $30\text{ mA}$).}
        \label{fig:rcd-foto}
    \end{minipage}\hfill
    \begin{minipage}{0.52\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/rcd_werking_schematisch.png}
        \caption{Interne opbouw van een RCD met somstroomtransformator, secundaire meetspoel, uitschakelrelais en testcircuit.}
        \label{fig:rcd-schematic}
    \end{minipage}
\end{figure}
\FloatBarrier

\textbf{Gedetailleerd werkingsprincipe (Somstroomtransformator):}
\begin{enumerate}
    \item Alle actieve geleiders (de fase(n) L1, L2, L3 én de nulleider N) worden als primaire wikkelingen
          door een ferromagnetische ringkern (\emph{torus}) gevoerd.
    \item \textbf{Normale toestand (geen lek):} De stroom die naar de verbruiker vloeit, keert integraal
          terug. De vectoriële som van de stromen is exact nul:
          \[\sum \underline{I} = \underline{I}_{L1} + \underline{I}_{L2} + \underline{I}_{L3} + \underline{I}_N = 0\]
          De magnetische velden die door de heengaande en terugkerende stromen worden opgewekt, heffen
          elkaar in de ringkern volkomen op: $\Phi_{\text{net}} = 0$.
    \item \textbf{Fouttoestand (isolatiedefect of aanraking):} Een deel van de stroom vloeit via het gestel,
          de aarding of het menselijk lichaam weg naar de aarde. De stroombalans is verstoord:
          \[\sum \underline{I} = \underline{I}_{\Delta} \neq 0\]
    \item Hierdoor ontstaat in de ringkern een resulterende wisselende magnetische flux $\Phi_{\text{net}}(t)$.
    \item Volgens de inductiewet van Faraday ($e = -N \frac{d\Phi}{dt}$) wekt deze flux een spanning op
          in een secundaire \keyterm{meetspoel}.
    \item Deze spanning stuurt een gevoelig elektromagnetisch uitschakelrelais (\emph{trip relay}) aan,
          dat mechanisch de hoofdcontacten ontgrendelt en binnen $<30\text{ ms}$ alle polen uitschakelt.
\end{enumerate}

\textbf{Functie van de testknop ($T$):}
Op elke RCD zit een mechanische testknop $T$ met een ingebouwde serieweerstand $R_T$. Wanneer je hierop
drukt, wordt er een stroompad gecreëerd dat \belangrijk{buiten de ringkern om} van een fase naar de nulleider
loopt. Dit simuleert een kunstmatige verschilstroom $I_\Delta$. Hiermee controleer je niet de aarding van het
gebouw, maar \emph{het mechanische en elektrische uitschakelmechanisme van het toestel zelf}. Het AREI
schrijft voor deze knop regelmatig (bv.\ maandelijks of halfjaarlijks) in te drukken om mechanische verkleving
te voorkomen.
...
\begin{itemize}
    ...
    \item \keyterm{Type A:} reageert op sinusvormige wisselstromen én \textbf{pulserende gelijkstromen}.
          Dit is de standaard in residentiële installaties (verplicht volgens AREI) omdat moderne huishoudtoestellen
          (wasmachines, pc-voedingen, dimmers, omvormers) elektronische gelijkrichters bevatten die gelijkstroomcomponenten
          kunnen lekken.
    \item \keyterm{Type B:} reageert op AC, pulserende DC én \textbf{gladde gelijkstromen} (smooth DC) tot hoge frequenties.
          Noodzakelijk bij PV-omvormers zonder transformator, frequentieregelaars en laadpalen voor elektrische voertuigen.
\end{itemize}

\textbf{Toepassing in het residentiële net (AREI):}
\begin{itemize}
    \item \keyterm{Algemene hoofddifferentieel ($300\text{ mA}$):} geplaatst aan het begin van het bord (dient voor brandveiligheid en algemene foutstroomdetectie).
    \item \keyterm{Gevoelige secundaire differentieel ($30\text{ mA}$):} verplicht voor alle kringen in \belangrijk{vochtige lokalen}
          en toestellen met watercontact: badkamers, wasmachine, droogkast, vaatwasser en buitenstopcontacten.
\end{itemize}
...
\begin{waarschuwing}[Essentiële beperkingen van een RCD (Waar een RCD níet tegen beschermt)]
    \begin{enumerate}
        \item Een RCD biedt \belangrijk{geen bescherming tegen overbelasting of kortsluiting} tussen fase en nul!
              ...
              en zal de RCD nooit trippen. Daarom moet een RCD altijd gecombineerd worden met een MCB (of uitgevoerd
              zijn als een gecombineerde \emph{RCBO}).
        \item Een RCD reageert \belangrijk{niet} wanneer een persoon gelijktijdig de fase én de nulgeleider aanraakt
              zonder contact te maken met de aarde. De stroom door het lichaam vloeit dan netjes van fase naar nul;
              voor de RCD is dit niet te onderscheiden van een normale lamp of broodrooster!
    \end{enumerate}
\end{waarschuwing}
