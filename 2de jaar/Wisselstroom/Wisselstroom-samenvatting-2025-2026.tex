...
Dit vak is voor negen tiende complexe rekenkunde. Als je je toestel goed kent, hoef je op het examen
bijna niets met de hand uit te werken --- en dat scheelt zowel tijd als rekenfouten. Zet dit dus
\'e\'en keer goed op v\'o\'or je begint te oefenen.
...
\begin{theorie}[De vier instellingen die je moet controleren]
    \begin{enumerate}
        \item \textbf{Complexe modus} aan (op de meeste toestellen \texttt{CMPLX} of \texttt{Complex}).
              Zonder dat geeft je toestel gewoon een foutmelding zodra er een $j$ in staat.
        \item \textbf{Graden of radialen}, en dan consequent. Cursussymbolen staan hier in graden
              ($\angle 30^\circ$), dus zet \texttt{DEG}. De klassieke blunder is een hoek in graden
              invoeren terwijl het toestel in \texttt{RAD} staat.
        \item \textbf{Weergavevorm}: kies of je het antwoord in $a+jb$ of in $r\angle\theta$ wil zien.
              Je kunt meestal per berekening omschakelen --- doe dat, want je hebt beide vormen nodig.
        \item \textbf{Vast aantal cijfers} (bv.\ \texttt{FIX 3} of \texttt{Norm 2}). Anders sleep je
              afrondingsfouten mee door een lange oefening.
    \end{enumerate}
\end{theorie}

\textbf{Welke vorm wanneer?} Je toestel rekent in beide, maar zelf denken blijft nodig:
...
\begin{theorie}[Functies die je in dit vak echt nodig hebt]
    \begin{itemize}
        ...
        \item $\texttt{Conjg}(\underline{I})$ --- de toegevoegde $\underline{I}^{*}$. Onmisbaar voor
              het complex vermogen $\underline{S} = \underline{V}\,\underline{I}^{*}$; vergeet die
...
Zowel knooppuntanalyse als lusanalyse eindigt in een stelsel lineaire vergelijkingen met complexe
co\"effici\"enten. Dat hoef je \textbf{niet} met de hand te elimineren.
...
\begin{theorie}[Wanneer is een stelsel oplosbaar?]
    ...
    onbekenden}. Tel dus altijd eerst, v\'o\'or je begint te rekenen:
    ...
    \begin{itemize}
        ...
        \item \textbf{Te veel} vergelijkingen $\rightarrow$ minstens \'e\'en ervan is een herhaling
              van de andere. Bij knooppuntanalyse is de KCL in de \emph{referentieknoop} altijd zo'n
              overbodige vergelijking: ze volgt automatisch uit de andere. Laat die dus weg.
    \end{itemize}
\end{theorie}

\belangrijk{``Onafhankelijk'' is het sleutelwoord.} Twee vergelijkingen die veelvouden van elkaar
zijn, tellen samen maar voor \'e\'en. Je toestel merkt dat zelf: het geeft dan een foutmelding
...
\begin{itemize}
    \item \textbf{Knooppuntanalyse:} het aantal knopen min \'e\'en (de referentieknoop). Elke knoop
          waar een spanningsbron rechtstreeks op de referentie aansluit, valt daar nog eens af ---
          die knoopspanning ken je al.
    \item \textbf{Lusanalyse:} het aantal ramen. Elk raam met een stroombron in een buitentak valt
          af --- die kringstroom ken je al.
\end{itemize}
...
\begin{theorie}[Werkwijze]
    \begin{enumerate}
        ...
        \item Groepeer de co\"effici\"enten \emph{voor} je begint in te tikken, bv.\
              $-j30 + j80 = j50$. Een fout hier vind je later nooit meer terug.
...
\begin{waarschuwing}[Aanvaardt jouw toestel complexe co\"effici\"enten?]
    ...
    Lukt het niet, dan is er een truc die altijd werkt: splits elke onbekende in een re\"eel en een
    imaginair deel ($\underline{I}_1 = x_1 + jy_1$), scheid elke vergelijking in haar re\"ele en haar
    imaginaire deel, en los het resulterende \textbf{re\"ele} stelsel van dubbele grootte op. Drie
    complexe vergelijkingen worden zo zes re\"ele.
\end{waarschuwing}

\begin{examenbox}[EXAMENTIP: laat je tussenstappen staan]
    Ook al doet je toestel het rekenwerk: schrijf de \textbf{opgestelde vergelijkingen} en de
    gebruikte formules wel op. Daar staan de punten op. Een kaal eindantwoord zonder stelsel levert
    weinig op, ook als het klopt. En teken je fasordiagram met de hand --- dat kan je toestel niet
    voor je doen.
\end{examenbox}
...
\begin{waarschuwing}[Twee valkuilen bij hoeken]
    \begin{itemize}
        \item Je toestel geeft $\arg$ altijd in het interval $(-180^\circ, 180^\circ]$. Fysisch kan de
              werkelijke hoek buiten dat bereik liggen; controleer zelf in welk kwadrant je antwoord
              hoort door naar de tekens van $a$ en $b$ te kijken.
        \item $\arctan(X/R)$ met de hand berekend geeft hetzelfde probleem: $\arctan$ kan het verschil
              tussen het tweede en het vierde kwadrant niet zien. Werk daarom liever rechtstreeks met
...
Dat mag omdat \emph{alle} grootheden in één oefening dezelfde $\omega$ hebben en dus even snel
draaien --- hun onderlinge hoeken liggen vast. Precies daarom mag je fasoren met \emph{verschillende}
...
\begin{figure}[htbp]
    ...
    \caption{Lengte $=$ amplitude, hoek met de re\"ele as $=$ fasehoek. In werkelijkheid draait de
        pijl met $\omega$, maar omdat alle fasoren even snel draaien tekenen we hem stil.}
    \label{fig:fasor}
\end{figure}
\FloatBarrier
...
\textbf{Admittantie: gewoon $1/\underline{Z}$.} Soms is delen door een impedantie lastig, vooral bij
takken die \textbf{parallel} staan. Dan keer je de breuk om en werk je met de \textbf{admittantie}
$\underline{Y}$. Parallelle admittanties mag je namelijk gewoon optellen, net zoals je
serie-impedanties optelt.
...
\begin{warningbox}[Schijnbaar vermogen of complex vermogen?]
    Twee namen die vlak bij elkaar liggen, maar niet hetzelfde zijn:
    \begin{itemize}
    ...
    \end{itemize}
    Beide staan in VA, vandaar de verwarring. Het verschil telt omdat alleen $\underline{S}$
    optelbaar is: $|\underline{S}_T| \neq |\underline{S}_1| + |\underline{S}_2|$, maar
...
als $\underline{S} = \sum(P_n + jQ_n)$. Dat is de enige reden waarom het complex vermogen bestaat.
Werkwijze bij elke opgave van dit type: zet elke last om naar $P + jQ$ met
\[Q = P\tan\phi \qquad\text{en}\qquad \phi = \arccos(\text{PF})\]
tel op (of trek af), en zet pas op het einde terug om naar $|S|$ en PF.
...
\begin{oefening}[Sadiku 11.74: twee lasten samentellen]
    ...
    \textbf{Kerninzicht.} Merk op hoe de arbeidsfactor van het geheel ($0{,}899$) \textbf{tussen} die
    van de twee lasten ligt, dichter bij de grootste. Je mag PF's dus nooit zomaar middelen --- ga
    altijd via $P+jQ$. En let op de grootte-orde: $5{,}74$~mF is enorm, omdat $120$~V erg laag is voor
    $64$~kW. Bij een hogere netspanning is dezelfde correctie veel goedkoper, want
    $C \sim 1/V^2$.
\end{oefening}
...
\begin{figure}[H]
    ...
    \caption{Spanningsbron \textbf{in serie} met $\underline{Z}$ $\leftrightarrow$ stroombron
        \textbf{parallel} aan diezelfde $\underline{Z}$. Aan de klemmen $a$--$b$ niet te
        onderscheiden. De pijl van de stroombron wijst naar de $+$-klem van de spanningsbron.}
    \label{fig:brontransformatie}
\end{figure}
\FloatBarrier

Voorwaarde: er \emph{moet} een impedantie bij horen. Een ideale bron (zonder $\underline{Z}$) kun je
niet omzetten.
...
\begin{voorbeeld}[Beide richtingen]
    ...
    $\underline{Z}$ eerst polair, want je gaat delen of vermenigvuldigen.
\end{voorbeeld}
...
\begin{oefening}[Sadiku Probleem 10.7: Brontransformatie en Stroomdeling]
    ...
    \begin{enumerate}
        ...
        \item \textbf{Omzetting naar Norton-stroombron:}
              ...
              De keten ziet er na die omzetting zo uit --- de bron is nu een \textbf{stroombron} met
              ...
              \begin{center}
                  ...
                  \end{circuitikz}
              \end{center}
              Nu staan $\underline{Z}_1$ en $j5\,\Omega$ zichtbaar parallel, en dat is precies wat
              stap 3 samenneemt.
...
\begin{oefening}[Sadiku 10.16: twee stroombronnen wegwerken met brontransformatie]
    ...
    \textbf{Denkstap.} Twee stroombronnen zijn lastig: je kunt niet zomaar een lus rondlopen. Maar
    elke stroombron \emph{met een impedantie ernaast} mag je omzetten naar een spanningsbron met
    diezelfde impedantie \emph{in serie}. Doe dat twee keer en je houdt \'e\'en enkele lus over ---
    en dan is het gewoon de wet van Ohm.
    ...
    omzetting haalt \'e\'en knooppunt uit het net weg. Zodra er maar \'e\'en lus over is heb je geen
    stelsel meer nodig. Zoek dus altijd eerst naar een bron met een impedantie \emph{ernaast} (bij
...
Knooppuntanalyse telt \emph{knopen}, lusanalyse telt \emph{lussen}. Ze lossen hetzelfde net op ---
kies gewoon degene die het \textbf{kleinste stelsel} geeft. Vuistregel: tel eerst het aantal
onbekende knoopspanningen en het aantal onafhankelijke lussen, en neem de kleinste van de twee.
...
\begin{theorie}[Werkwijze]
    \begin{enumerate}
        ...
        \item Loopt er een \textbf{stroombron} in een tak die maar bij \'e\'en lus hoort, dan is die
              kringstroom meteen bekend. Je hoeft er dan geen vergelijking meer voor te schrijven.
...
\belangrijk{Een kringstroom is geen echte stroom.} Ze is een rekenhulp. Alleen in een tak aan de
...
\begin{oefening}[Voorbeeld uit de slides: stromen zoeken met lussen]
    ...
    \emph{alleen} in de linkerlus, dus die kringstroom ligt al vast:
    $\underline{I}_s = 3\angle 60^\circ$~A. Blijven er twee over. Merk ook op dat de $60\,\Omega$ en
    de $j80\,\Omega$ \textbf{gedeelde} takken zijn --- daar komt straks het verschil van twee
    kringstromen in.
    ...
    \textbf{Kerninzicht.} Beide kringstromen komen hier uit op \emph{dezelfde} hoek als de bron
    ($60^\circ$), omdat de $j$'s in lus 2 tegen elkaar wegdelen: $j80/j50$ is een re\"eel getal. Dat
    is toeval van deze getallen --- in het algemeen krijg je wel degelijk een faseverschuiving.
    Merk verder op dat $\underline{I}_n$ exact gelijk is aan de bronstroom; ook dat volgt hier
...
\begin{waarschuwing}[Knooppunt- of lusanalyse: welke kies je?]
    \begin{itemize}
        ...
        \item Een stroombron in een \emph{buitentak} maakt \'e\'en kringstroom gratis bekend; een
              spanningsbron rechtstreeks aan de referentieknoop maakt \'e\'en knoopspanning gratis
              bekend. Kijk dus altijd eerst welke bronnen je een vergelijking besparen.
    \end{itemize}
\end{waarschuwing}
...
\begin{oefening}[Sadiku 11.85: driedradennet met drie lussen --- werk met SysSolve]
    ...
    \textbf{Denkstap.} Drie ramen, geen enkele stroombron, dus drie volwaardige KVL-vergelijkingen
    met complexe co\"effici\"enten. Met de hand elimineren duurt lang en gaat gegarandeerd mis ---
    dit is precies waar je \texttt{SysSolve} voor gebruikt.
    ...
    \textbf{onevenwichtig} belast (een lamp tegenover een koelkast), dus de nulgeleider voert wel
    degelijk stroom --- $|I_n| = 9{,}6$~A. Precies de reden waarom je een nulgeleider nooit mag
    zekeren of onderbreken. Controleer je antwoord trouwens met KCL in het middelste knooppunt:
...
\begin{oefening}[Sadiku 10.46: superpositie met drie verschillende frequenties]
    ...
    \textbf{Denkstap.} Er staan \textbf{drie bronnen met drie verschillende pulsaties} in dit net:
    $\omega = 0$ (de 15~V DC), $\omega = 2$ en $\omega = 3$~rad/s. Een fasor hoort altijd bij
    \'e\'en frequentie, dus je \emph{mag} deze drie niet in \'e\'en fasordiagram gooien. Reken elke
    bron apart uit op haar eigen frequentie, zet elk deelresultaat om naar de tijd, en tel pas
    \textbf{daarna} op.

    \belangrijk{De impedanties veranderen mee met $\omega$}, dus je moet $\underline{Z}_L$ en
    $\underline{Z}_C$ per geval opnieuw uitrekenen. Dat is precies waar deze oefening op mikt.
    ...
    \begin{center}
        ...
        \end{circuitikz}
    \end{center}
    \stapreden{Door de $6\,\Omega$ loopt wel stroom, maar de spoel is een draad, dus het knooppunt van $v_o$ hangt rechtstreeks aan de bron:}
    \[v_{o,1} = 15\ \text{V}\]
    ...
    \textbf{Kerninzicht.} De drie deelantwoorden hebben elk een andere frequentie, dus er is
    \emph{geen} enkele fasor die het geheel beschrijft --- en ook geen enkele rms-waarde die je
    zomaar mag optellen. Wil je toch de effectieve waarde van $v_o$, dan tel je \textbf{kwadratisch}
    op:
...
\begin{oefening}[Alleen groottes gegeven: de cosinusregel op een KCL-knoop]
    ...
    \emph{vectoriële} som, dus de drie fasoren sluiten kop-staart aan tot een \textbf{driehoek}. En
    van een driehoek waarvan je de drie zijden kent, liggen de hoeken vast --- daar is de
    cosinusregel voor. Je hebt dus helemaal geen hoekinformatie nodig.
...
Op slides en examens duikt de sterschakeling in allerlei gedaanten op. Laat je daar niet door
misleiden: het is telkens hetzelfde net.
...
\begin{figure}[H]
    ...
    \caption{Dezelfde \textbf{sterschakeling}, vijf keer anders getekend (bronnen links, lasten
        ...
        alle drie de takken samenkomen --- het sterpunt. Of dat nu een verticale rail is, een
        letterlijke Y of een onderste verzamellijn maakt elektrisch geen verschil. Zolang dat punt
        bestaat geldt $V_L = \sqrt{3}V_f$ en $I_L = I_f$.}
...
\begin{figure}[H]
    ...
    \caption{Dezelfde \textbf{driehoekschakeling}, vijf keer anders getekend (bronnen links, lasten
        rechts). Het herkenningspunt is hier: \'e\'en \textbf{gesloten lus} van drie takken, met
        \'e\'en aansluitpunt per hoek --- er is \emph{geen} sterpunt. Elke tak ligt daardoor
        rechtstreeks tussen twee lijnen, dus $V_f = V_L$ en $I_L = \sqrt{3}I_f$.}
...
Bijna elke 3-fase oefening los je op met hetzelfde recept. Je herleidt het net tot het
\textbf{eenfase-equivalent}: één bron, één impedantie, één lus. Alles wat daarvoor nodig is, is
alles naar \textbf{ster} omzetten.
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Het recept voor vrijwel elke 3-fase oefening. Stap 1 t.e.m.\ 3 zijn puur omzetwerk;
        vanaf stap 4 is het gewoon een eenvoudige serieschakeling.}
...
\begin{waarschuwing}[Wat je op het einde nog moet terugrekenen]
    ...
    de oorspronkelijke driehoek, dan moet je terug:
...
driehoek en hun impedantie wordt 3$\times$ kleiner: 3$\times$ meer stroom en 3$\times$ meer vermogen.
Starten in ster (zacht), draaien in driehoek (vol vermogen).
...
{Dezelfde symbolen. Teller is telkens \emph{dezelfde} som; enkel de noemer wisselt: de
...
\begin{oefenblok}[$\Delta$-bron, twee parallelle lasten --- alle zes stappen]
    ...
    \textbf{Kerninzicht.} De $\Delta$-bron is nooit echt omgezet naar drie bronnen --- je hebt alleen
    de \emph{spanning} herrekend. Dat volstaat, want het eenfase-equivalent heeft enkel
    $\underline{V}_{an}$ nodig. Diezelfde $-30^\circ$ komt overal terug: van lijn- naar
    fasespanning.
\end{oefenblok}
...
Bij een evenwichtige last is het totale vermogen simpelweg $P_{tot} = 3P_{fase}$. Zodra de drie
takken verschillen valt die truc weg, en meet je met wattmeters. De 2-wattmethode zegt dat je er
daarvoor maar \textbf{twee} nodig hebt in plaats van drie.

\subsubsection{Het idee in drie zinnen}

\begin{theorie}[Waarom twee wattmeters volstaan]
    Het totale vermogen is per definitie de som van de drie \textbf{fasevermogens}, elk gemeten
    t.o.v.\ het sterpunt $n$. Maar in een driedradensysteem is $n$ er niet, dus nemen we
    \'e\'en van de lijnen (zeg $b$) als referentie. Daardoor worden de fasespanningen
    \textbf{lijnspanningen} --- en omdat $\underline{I}_a + \underline{I}_b + \underline{I}_c = 0$
    verdwijnt de derde meter vanzelf uit de som. Wat overblijft is
    $P_{tot} = W_1 + W_2$: twee lijnspanningen maal twee lijnstromen.
\end{theorie}
...
driehoeklast is $I_f = I_L/\sqrt{3}$, dus dat verschil is geen detail.
...
\begin{waarschuwing}[Wanneer twee wattmeters níet volstaan]
    ...
    (vierdraads en onevenwichtig). De afleiding steunt op $\underline{I}_a + \underline{I}_b +
        \underline{I}_c = 0$, en die som is dan niet meer nul. Dan heb je wél \textbf{drie} wattmeters nodig.
\end{waarschuwing}

\subsubsection{De hoeken: waar komen die $30^\circ$ vandaan?}

Voor een \textbf{evenwichtige} last met impedantiehoek $\theta$ ($\theta>0$ = inductief) liggen de
twee hoeken vast. Lees ze af op het fasordiagram: $\underline{V}_{ab}$ ijlt $30^\circ$ voor op
$\underline{V}_{an}$, en $\underline{I}_a$ ijlt $\theta$ na op $\underline{V}_{an}$ --- samen dus
$\theta + 30^\circ$. Voor $\underline{V}_{cb}$ en $\underline{I}_c$ werkt het net de andere kant op.
...
\begin{waarschuwing}[Alleen $W_1+W_2$ geldt altijd]
    ...
    Bij een ongebalanceerde last zijn ze gewoon ongeldig --- je krijgt er zelfs verkeerde tekens mee.
...
\begin{theorie}[Wat zeggen de aflezingen over $\cos\theta$?]
    ...
    \begin{itemize}
        ...
        \item $\theta = 60^\circ$ ($\cos\theta = 0{,}5$): $W_1 = 0$, \textbf{meter 1 staat stil} en
              $W_2 = P_{tot}$.
        \item $\theta > 60^\circ$: $W_1 < 0$, de wijzer slaat terug. Om af te lezen pool je de
              spanningsspoel om; $P_{tot}$ blijft algebraïsch $W_1 + W_2$.
        \item $\theta = 90^\circ$ (zuiver reactief): $W_1 = -W_2$, dus $P_{tot} = 0$.
    \end{itemize}
    Bij een \textbf{capacitieve} last ($\theta < 0$) draait het hele plaatje om: dan is $W_1$ de
    grote en $W_2$ de kleine (of negatieve) aflezing.
\end{theorie}
...
\begin{oefening}[Slides hoofdstuk 4, 45--47]
    ...
    \textbf{Denkstap.} De bron staat in ster, de last in driehoek. De last ``ziet'' dus de
    \emph{lijn}spanning over elke $\underline{Z}_p$. En de wattmeters meten sowieso lijngrootheden,
    dus alles wat we nodig hebben is $V_L$ en $I_L$.

    \stapreden{Bij een positief sequent stersysteem ijlt de lijnspanning $\sqrt{3}$ keer groter en $30^\circ$ voor:}
    ...
    \textbf{Kerninzicht.} De last is \emph{capacitief} ($\theta = -56{,}31^\circ$), en dat zie je
    meteen aan de aflezingen: $W_1$ is groot, $W_2$ bijna nul. Bij een inductieve last was het net
...
\begin{theorie}[Werkwijze bij een ongebalanceerde ster]
    ...
    \begin{enumerate}
        ...
        \item \textbf{Zonder nulgeleider.} Het sterpunt $N'$ zweeft. Bereken eerst de verschuiving
              $\underline{V}_{N'N}$ met Millman (hieronder), trek die van elke fasespanning af, en
              reken pas dan de takstromen.
...
Dit is de tabel die je op het examen in je hoofd wil hebben. De hele truc is: \textbf{onthou welke
    grootheid gelijk blijft}, dan volgt de $\sqrt{3}$ vanzelf voor de andere.
...
\belangrijk{Dezelfde last in driehoek trekt dus drie keer zoveel vermogen als in ster.} Dat is precies
het principe achter de \textbf{ster-driehoekstarter} van een motor: aanlopen in ster (minder stroom
en koppel), en pas in regime overschakelen naar driehoek (vol vermogen).
...
vertraagd; een kleine overstroom mag immers gedurende korte tijd blijven bestaan.
...
impedantie. Een MCB detecteert beide, maar met een verschillend mechanisme en een
verschillende uitschakelsnelheid. Een MCB vervangt geen RCD: de MCB beschermt de
installatie tegen overstroom, terwijl de RCD beschermt tegen lekstromen naar aarde.
