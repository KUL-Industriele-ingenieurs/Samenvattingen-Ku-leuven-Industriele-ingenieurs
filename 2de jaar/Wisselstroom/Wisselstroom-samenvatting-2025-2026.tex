...
\subsection{Overzicht bewijzen (voorkennis Elektriciteit)}

\begin{waarschuwing}[Voorbehoud]
    Deze lijst komt van het studforum en hoort bij het \belangrijk{oude lesprogramma} (van vóór de
    hervorming rond 2021, toen Trillingen en Golven nog bij dit vak zat). Reken er niet op dat je
    deze bewijzen moet kennen. Ze staan hier als achtergrond en als oefenmateriaal --- de
    onderwerpen zelf ($\cos\phi$-correctie, wattmeter, driefasesystemen) zijn wél nog steeds
    examenstof.
\end{waarschuwing}

Onderstaande bewijzen komen uit de voorkennis van Elektriciteit (bach1):
...
Deze samenvatting bundelt alle essentiële theorie, formules, en uitgewerkte examenoefeningen voor het vak \textbf{Wisselstroomnetten} (2de bachelor Industriële Ingenieurswetenschappen, KU Leuven). Aangezien er op het examen geen formularium ter beschikking wordt gesteld, zijn alle nodige afleidingen, vuistregels en controlemethoden puntsgewijs samengebracht.
...
\begin{itemize}
    \item \textbf{Zuivere sinus:} $k_f = \frac{A_m / \sqrt{2}}{2A_m / \pi} = \frac{\pi}{2\sqrt{2}} \approx \mathbf{1{,}11}$ \quad en \quad $k_p = \frac{A_m}{A_m / \sqrt{2}} = \sqrt{2} \approx \mathbf{1{,}414}$.
    \item \textbf{Enkelzijdig gelijkgerichte sinus:} $k_f = \frac{A_m / 2}{A_m / \pi} = \frac{\pi}{2} \approx \mathbf{1{,}57}$ \quad en \quad $k_p = \frac{A_m}{A_m / 2} = \mathbf{2}$.
    \item \textbf{Blokgolf:} $k_f = \mathbf{1{,}0}$ \quad en \quad $k_p = \mathbf{1{,}0}$.
    \item \textbf{Driehoeksgolf / zaagtand:} $k_f = \frac{A_m / \sqrt{3}}{A_m / 2} = \frac{2}{\sqrt{3}} \approx \mathbf{1{,}155}$ \quad en \quad $k_p = \sqrt{3} \approx \mathbf{1{,}732}$.
\end{itemize}
...
\begin{itemize}
    \item \keyterm{Klassieke draaispoelmeter met gelijkrichter:} meet mechanisch de \textbf{gemiddelde gelijkgerichte waarde} $A_{\text{gem}}$ en vermenigvuldigt die intern met de vaste factor $\mathbf{1{,}11}$ (de vormfactor van een zuivere sinus). Bij een afwijkende golfvorm (bv.\ dimmer, gelijkrichter) wijst dit toestel dus een \emph{foutieve} effectieve waarde aan!
    \item \keyterm{Draai-ijzer- / weekijzermeter:} de mechanische afbuiging is evenredig met het kwadraat van de stroom ($i^2$). Hierdoor meet het toestel rechtstreeks de \textbf{echte effectieve waarde} $A_{\text{eff}}$, onafhankelijk van de golfvorm (voor lage netfrequenties).
    \item \keyterm{True RMS digitale meter:} bemonstert het signaal digitaal met hoge frequentie en berekent numeriek $\sqrt{\frac{1}{T}\int_0^T a^2(t)\,dt}$. Geeft altijd de correcte effectieve waarde.
\end{itemize}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=1.25]
        ...
        % Grid lijnen
        \draw[gray!25, thin] (-2.5,-2.5) grid (2.5,2.5);

        % Fasor vector (bijvoorbeeld 45 graden)
        \draw[->, very thick, blue] (0,0) -- (2,2);

        % Hoek tonen
        \draw[red, thick] (0,0) -- (1.2,0) arc (0:45:1.2);
        \node[red] at (0.95,0.42) {$\phi$};

        % Projecties
        \draw[dashed, blue!60] (2,2) -- (2,0) node[below, yshift=-0.12cm] {$V\cos\phi$};
        \draw[dashed, blue!60] (2,2) -- (0,2) node[left, xshift=-0.12cm] {$V\sin\phi$};

        % Labels - positioned away from the vector
        \node[blue, anchor=west] at (2.05,2.15) {$V_m\angle\phi$};
        \node[blue] at (2.15,1.75) {$\vec{V}$};
    \end{tikzpicture}
...
\begin{waarschuwing}[Eerst alles naar dezelfde functie]
    Je mag pas fasoren optellen als alle termen \belangrijk{dezelfde golfvorm} hebben, dus allemaal
    sinus of allemaal cosinus. Gebruik daarvoor:
    \[\sin\theta = \cos(\theta - 90^\circ) \qquad\qquad \cos\theta = \sin(\theta + 90^\circ)\]
    Een uitdrukking als $5\cos(3t+7^\circ) + 2\sin(3t-28^\circ)$ mag je dus \belangrijk{niet}
    rechtstreeks als $5\angle 7^\circ + 2\angle -28^\circ$ schrijven. Zet eerst de sinus om.
    \emph{Dit was letterlijk theorievraag 1 op 20/01/2024 én 22/08/2024.}
\end{waarschuwing}

\begin{waarschuwing}[Optellen mag alleen bij dezelfde frequentie]
    Fasoren van signalen met een \belangrijk{verschillende} $\omega$ mag je nooit optellen: ze
    draaien met verschillende snelheid, dus hun onderlinge hoek ligt niet vast. Heeft een schema
    meerdere bronnen met verschillende frequenties, dan moet je met \keyterm{superpositie} werken:
    reken elke bron apart uit bij zijn eigen $\omega$ en tel de resultaten pas op in het
    \emph{tijdsdomein}.
\end{waarschuwing}

\begin{examenbox}[Amplitude of effectieve waarde?]
    Een fasor kan je met de \textbf{amplitude} $A_m$ of met de \textbf{effectieve waarde}
    $A_{rms} = A_m/\sqrt{2}$ schrijven. Beide mag, zolang je \belangrijk{consequent} bent.

    Let op waar je welke nodig hebt:
    \begin{itemize}
        \item \keyterm{Vermogens} ($P$, $Q$, $S$) reken je altijd met \textbf{effectieve} waarden.
        \item Het \keyterm{ogenblikkelijk vermogen} $p(t) = v(t)i(t)$ reken je met de
...
\begin{oefening}[Gemiddelde en effectieve stroom]
    ...
    Eerst de piekstroom uit de wet van Ohm, met $V_m = \sqrt{2} \cdot V_{rms}$:
...
\begin{itemize}
    \item \keyterm{ELI} --- bij een spoel ($L$) komt $E$ vóór $I$: de \textbf{spanning ijlt voor} op
          de stroom, of omgekeerd gezegd, de stroom \textbf{ijlt na}. Een spoel is dus
          \textbf{naijlend} (Engels: \emph{lagging}).
    \item \keyterm{ICE} --- bij een condensator ($C$) komt $I$ vóór $E$: de \textbf{stroom ijlt voor}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=0.92, line cap=round, line join=round]
        ...
        \begin{scope}[shift={(0,0)}]
            % Titel
            \node[font=\bfseries\sffamily, text=schoolBlue!90!black, anchor=west] at (-0.3, 2.35)
            {\textbf{1. Zuivere spoel $L$ (ELI: Spanning $v$ ijlt $90^\circ$ voor op stroom $i_L$)}};

            % Assen
            \draw[->, thick] (-0.3,0) -- (7.2,0) node[right, font=\footnotesize] {$\omega t$};
            \draw[->, thick] (0,-1.65) -- (0,1.95) node[above, font=\footnotesize] {$v(t), i_L(t)$};

            % Verticale rasterlijnen
            \draw[gray!30, dotted, thick] (1.571,-1.4) -- (1.571,1.4);
            \draw[gray!30, dotted, thick] (3.142,-1.4) -- (3.142,1.4);
            \draw[gray!30, dotted, thick] (4.712,-1.4) -- (4.712,1.4);
            \draw[gray!30, dotted, thick] (6.283,-1.4) -- (6.283,1.4);

            % Labels op x-as
            \node[below, font=\scriptsize] at (1.571, -0.05) {$\frac{\pi}{2}$ ($90^\circ$)};
            \node[below, font=\scriptsize] at (3.142, -0.05) {$\pi$ ($180^\circ$)};
            \node[below, font=\scriptsize] at (4.712, -0.05) {$\frac{3\pi}{2}$ ($270^\circ$)};
            \node[below, font=\scriptsize] at (6.283, -0.05) {$2\pi$ ($360^\circ$)};

            % Spanning v(t) = 1.35 sin(x)
            \draw[domain=0:6.6, samples=120, smooth, ultra thick, schoolBlue]
            plot (\x, {1.35*sin(\x r)});

            % Stroom i_L(t) = 1.05 sin(x - 90) = -1.05 cos(x)
            \draw[domain=0:6.6, samples=120, smooth, ultra thick, schoolRed, dashed]
            plot (\x, {-1.05*cos(\x r)});

            % Piekmarkeringen & faseverschuiving
            \draw[schoolBlue, fill=schoolBlue] (1.571, 1.35) circle (0.045);
            \draw[schoolRed, fill=schoolRed] (3.142, 1.05) circle (0.045);

            \draw[<->, thick, schoolGreen] (1.571, 1.55) -- (3.142, 1.55)
            node[midway, above, font=\scriptsize\bfseries, text=schoolGreen] {Faseverschuiving $\Delta \phi = 90^\circ$ (stroom ijlt na / \emph{lagging})};

            % Legenda blok onderaan
            \draw[draw=gray!35, fill=white, rounded corners=2pt] (0.2, -1.05) rectangle (4.4, -1.65);
            \draw[ultra thick, schoolBlue] (0.4, -1.22) -- (0.9, -1.22) node[right, font=\scriptsize, text=black] {$v(t) = V_m\sin(\omega t)$};
            \draw[ultra thick, schoolRed, dashed] (0.4, -1.48) -- (0.9, -1.48) node[right, font=\scriptsize, text=black] {$i_L(t) = I_m\sin(\omega t - 90^\circ)$};
        \end{scope}

        % --- FASORDIAGRAM SPOEL ---
        \begin{scope}[shift={(9.6,0)}]
            \node[font=\bfseries\sffamily, text=schoolBlue!90!black] at (0.3, 2.35) {\textbf{Fasordiagram}};

            % Assenstelsel
            \draw[->, thick] (-1.2,0) -- (2.3,0) node[right, font=\footnotesize] {Re};
            \draw[->, thick] (0,-1.65) -- (0,1.95) node[above, font=\footnotesize] {Im};
            \draw[gray!20, thin] (-1.0,-1.4) grid (2.0,1.6);

            % Spanning fasor (0 graden)
            \draw[->, ultra thick, schoolBlue] (0,0) -- (1.8,0)
            node[above right, font=\footnotesize\bfseries] {$\vec{V} = V\angle 0^\circ$};

            % Stroom fasor (-90 graden)
            \draw[->, ultra thick, schoolRed] (0,0) -- (0,-1.35)
            node[below right, font=\footnotesize\bfseries] {$\vec{I}_L = I_L\angle -90^\circ$};

            % Oorsprong & hoekboog
            \fill (0,0) circle (0.05);
            \draw[->, thick, schoolGreen] (0.5,0) arc (0:-90:0.5);
            \node[schoolGreen, font=\scriptsize\bfseries] at (0.8, -0.38) {$-90^\circ$};
            \node[font=\scriptsize, align=center, text=gray!80!black] at (0.5, 1.25) {$\vec{I}_L$ ijlt $90^\circ$ na\\(\emph{lagging})};
        \end{scope}
    \end{tikzpicture}

    \vspace{0.6cm}

    % === DEEL 2: CONDENSATOR C (ICE) ===
    \begin{tikzpicture}[scale=0.92, line cap=round, line join=round]
        % --- GOLFVORMEN CONDENSATOR ---
        \begin{scope}[shift={(0,0)}]
            % Titel
            \node[font=\bfseries\sffamily, text=schoolBlue!90!black, anchor=west] at (-0.3, 2.35)
            {\textbf{2. Zuivere condensator $C$ (ICE: Stroom $i_C$ ijlt $90^\circ$ voor op spanning $v$)}};

            % Assen
            \draw[->, thick] (-0.3,0) -- (7.2,0) node[right, font=\footnotesize] {$\omega t$};
            \draw[->, thick] (0,-1.65) -- (0,1.95) node[above, font=\footnotesize] {$v(t), i_C(t)$};
            ...
            % Stroom i_C(t) = 1.05 sin(x + 90) = 1.05 cos(x)
            \draw[domain=0:6.6, samples=120, smooth, ultra thick, schoolRed, dashed]
            plot (\x, {1.05*cos(\x r)});
            ...
            \draw[<->, thick, schoolGreen] (0, 1.55) -- (1.571, 1.55)
            node[midway, above, font=\scriptsize\bfseries, text=schoolGreen] {Faseverschuiving $\Delta \phi = 90^\circ$ (stroom ijlt voor / \emph{leading})};

            % Legenda blok onderaan
            \draw[draw=gray!35, fill=white, rounded corners=2pt] (2.7, -1.05) rectangle (6.9, -1.65);
            \draw[ultra thick, schoolBlue] (2.9, -1.22) -- (3.4, -1.22) node[right, font=\scriptsize, text=black] {$v(t) = V_m\sin(\omega t)$};
            \draw[ultra thick, schoolRed, dashed] (2.9, -1.48) -- (3.4, -1.48) node[right, font=\scriptsize, text=black] {$i_C(t) = I_m\sin(\omega t + 90^\circ)$};
        \end{scope}

        % --- FASORDIAGRAM CONDENSATOR ---
        \begin{scope}[shift={(9.6,0)}]
            \node[font=\bfseries\sffamily, text=schoolBlue!90!black] at (0.3, 2.35) {\textbf{Fasordiagram}};
            ...
            % Spanning fasor (0 graden)
            \draw[->, ultra thick, schoolBlue] (0,0) -- (1.8,0)
            node[below right, font=\footnotesize\bfseries] {$\vec{V} = V\angle 0^\circ$};

            % Stroom fasor (+90 graden)
            \draw[->, ultra thick, schoolRed] (0,0) -- (0,1.35)
            node[above right, font=\footnotesize\bfseries] {$\vec{I}_C = I_C\angle +90^\circ$};

            % Oorsprong & hoekboog
            \fill (0,0) circle (0.05);
            \draw[->, thick, schoolGreen] (0.5,0) arc (0:90:0.5);
            \node[schoolGreen, font=\scriptsize\bfseries] at (0.8, 0.38) {$+90^\circ$};
            \node[font=\scriptsize, align=center, text=gray!80!black] at (0.5, -1.25) {$\vec{I}_C$ ijlt $90^\circ$ voor\\(\emph{leading})};
        \end{scope}
    \end{tikzpicture}
    \caption{Vergelijking van de faseverschuiving ($90^\circ$) tussen spanning $v(t)$ en stroom $i(t)$ met bijhorende fasordiagrammen. Boven: bij een zuivere spoel $L$ bereikt $v(t)$ haar maximum $90^\circ$ vóór $i_L(t)$ (ELI: stroom ijlt na). Onder: bij een zuivere condensator $C$ bereikt $i_C(t)$ haar maximum $90^\circ$ vóór $v(t)$ (ICE: stroom ijlt voor).}
...
We beginnen met het eenvoudigste geval: een \keyterm{zuiver ohmse} belasting, waarbij spanning
...
We willen dit uitdrukken in effectieve waarden in plaats van in de piekwaarden $V_m$ en $I_m$,
want dat zijn de waarden die je meet.

\[V_{rms} = \frac{V_m}{\sqrt{2}} \quad\Longleftrightarrow\quad V_m = \sqrt{2}\,V_{rms}\]
\[I_{rms} = \frac{I_m}{\sqrt{2}} \quad\Longleftrightarrow\quad I_m = \sqrt{2}\,I_{rms}\]
\[\frac{V_m I_m}{2} = \frac{\sqrt{2}\,V_{rms} \cdot \sqrt{2}\,I_{rms}}{2} = V_{rms} I_{rms}\]

Ingevuld geeft dat:
\[p(t) = V_{rms} I_{rms}\left(1 - \cos(2\omega t)\right)\]

Twee dingen vallen op. Het \keyterm{gemiddelde} van $p(t)$ is $V_{rms}I_{rms}$, want de cosinusterm
heeft gemiddelde nul. En $p(t)$ wordt hier \belangrijk{nooit negatief}: een weerstand geeft geen
...
Voor een algemene belasting met fasehoek $\phi$ (bv.\ een $RL$- of $RC$-keten) luidt het \keyterm{ogenblikkelijk vermogen}:
...
\frm{reactief vermogen}{Q = V_{rms} I_{rms} \sin \phi}{Met Q het reactief vermogen in Volt-Ampère reactief (VAR), $V_{rms}$
    de effectieve spanning in Volt, $I_{rms}$ de effectieve stroom in Ampère en $\phi$ de fasehoek tussen spanning en stroom in graden of radialen.}
...
\frm{Vermogendriehoek}{\underline{S} = P + jQ}{Met S het schijnbaar vermogen in Volt-Ampère (VA), P het actieve vermogen
...
{Met $\underline{S}$ het complex vermogen in VA, $\underline{V}$ de spanningsfasor in V,
    ...
    vermogen in W en $Q$ het reactief vermogen in VAR.}

\begin{waarschuwing}[Waarom de toegevoegde?]
    Je wil de hoek \emph{tussen} $\underline{V}$ en $\underline{I}$, dus
    $\phi = \phi_V - \phi_I$. Bij een gewone vermenigvuldiging tellen de hoeken op
    ($\phi_V + \phi_I$), en dat is niet wat je zoekt. Door de \belangrijk{toegevoegde} te nemen keer
    je het teken van $\phi_I$ om, en dan geeft de vermenigvuldiging precies $\phi_V - \phi_I$.
    Vergeet je de ster, dan krijg je een verkeerde $\phi$ en dus een verkeerde $P$ en $Q$.
\end{waarschuwing}

\begin{examenbox}[Meerdere belastingen samen]
    Hangen er meerdere belastingen op dezelfde bron, dan tel je \belangrijk{$P$ en $Q$ apart} op:
    ...
    $\cos\phi_T$ is \belangrijk{niet} het gemiddelde van de arbeidsfactoren. Schijnbare vermogens
    zijn vectoren; alleen $P$ en $Q$ zijn scalair optelbaar.
\end{examenbox}

Het teken van $Q$ vertelt je meteen wat voor last je hebt:
\begin{itemize}
    \item $Q > 0$: \keyterm{inductief} (naijlend), de stroom loopt achter op de spanning;
    \item $Q < 0$: \keyterm{capacitief} (voorijlend), de stroom loopt voor op de spanning;
...
Dit is vaak de \keyterm{snelste} weg bij ongebalanceerde 3-fase oefeningen: je berekent per tak de
fasestroom en telt daarna gewoon alle $I^2R$ en $I^2X$ op.

\begin{examenbox}[Eindcontrole die je altijd moet doen]
    Alleen \belangrijk{weerstanden} verbruiken actief vermogen. Spoelen en condensatoren slaan
...
\begin{oefening}[Voorbeeldberekening: Vermogens en impedantie]
    ...
    \textbf{Oplossing:}
    \begin{itemize}
        \item Faseverschil: $\phi = 85^\circ - 15^\circ = 70^\circ$ (stroom ijlt na: inductief).
        \item $S = V_{\text{eff}} \cdot I_{\text{eff}} = 110 \cdot 0{,}4 = \mathbf{44\text{ VA}}$.
        \item $P = S \cos 70^\circ = 44 \cdot 0{,}342 = \mathbf{15{,}05\text{ W}}$.
        \item $Q = S \sin 70^\circ = 44 \cdot 0{,}9397 = \mathbf{41{,}35\text{ VAR}}$ (inductief).
        \item $\cos\phi = \frac{P}{S} = \mathbf{0{,}342}$ (naijlend).
        \item $\underline{Z} = \frac{\underline{V}}{\underline{I}} = \frac{110\angle 85^\circ}{0{,}4\angle 15^\circ} = \mathbf{275\angle 70^\circ\,\Omega = 94{,}06 + j258{,}4\,\Omega}$.
    \end{itemize}
\end{oefening}
...
\keyterm{De oplossing:} zet een condensator parallel over de installatie. Die levert het reactief
...
\begin{examenbox}
    Er bestaat een \textbf{tweede variant} van deze vraag: \textit{een installatie draait op vollast
        en men wil er een machine met vermogen $P'$ bijzetten \belangrijk{zonder} dat de stroom stijgt.}
    Dan ligt $S$ vast in plaats van $P$, en wordt het:
    \[\cos\phi_2 = \frac{P + P'}{S} \qquad\text{en}\qquad |Q_C| = S\left(\sin|\phi_1| - \sin|\phi_2|\right)\]
    \keyterm{Waarom hier $\sin$ en niet $\tan$?} Omdat het werkpunt nu langs een \emph{cirkel} met
...
\begin{waarschuwing}[Niet overcompenseren]
    Ga niet tot $\cos\phi = 1$. Bij lichte last wordt de installatie dan \belangrijk{capacitief}
...
\begin{waarschuwing}[De wortelformule geldt niet altijd]
    Die formule geldt alleen voor een \belangrijk{zuivere} serie- of parallelschakeling van $L$ en
    $C$. Zodra er een weerstand \emph{binnen} de parallelschakeling zit, verandert de effectieve
    reactantie en klopt ze niet meer.

    \keyterm{De algemene voorwaarde is altijd:}
...
\begin{itemize}
    \item $\omega < \omega_0$: bij een \keyterm{serie}kring capacitief, bij een
          \keyterm{parallel}kring inductief;
...
{Voor een tak met een reële spoel ($R_L$ in serie met $L$) parallel aan een ideale condensator $C$.
    Bij resonantie is de totale impedantie maximaal en zuiver reëel: $\underline{Z}_{\text{tot}} = R_p$ (de \keyterm{dynamische weerstand}).
    De stroom door de spoel en condensator is $Q$ keer groter dan de bronstroom (\keyterm{stroomopslingering}):
...
De boodschap van dit hoofdstuk is eenvoudig: \keyterm{alle netwerkanalyse die je vorig jaar bij
...
Het enige verschil is dat je nu met \belangrijk{complexe} impedanties $\underline{Z}$ rekent in
...
rekent met een schaalfactor, wanneer je knooppuntanalyse gebruikt, en hoe je een oefening aanpakt
...
\begin{waarschuwing}[Let op welke impedantie in de teller staat]
    Bij \textbf{spanningsdeling} staat in de teller de \belangrijk{eigen} impedantie.
    Bij \textbf{stroomdeling} staat in de teller de impedantie van de \belangrijk{andere} tak.
...
\subsubsection{Methode 1: achterwaarts rekenen met een schaalfactor}

Gebruik dit bij \keyterm{één bron} en een net dat een ketting is van serie- en
parallelverbindingen. Vooruit rekenen zou een stelsel geven; achteruit rekenen is één rechte lijn.

\begin{theorie}[Werkwijze]
    \begin{enumerate}
        \item Neem \emph{achteraan} in het net een hulpwaarde aan, bijvoorbeeld
              $\underline{V}_b = 100\angle 0^\circ$~V over de laatste tak.
        \item Reken van achter naar voor: takstromen met de wet van Ohm, samenvoegen met KCL,
              spanningsvallen erbij optellen tot je aan de bron komt.
        \item Je vindt zo een \emph{hulp}bronspanning $\underline{V}_s^{\text{hulp}}$.
        \item Bereken de schaalfactor
              \[\underline{K} = \frac{\underline{V}_s^{\text{werkelijk}}}{\underline{V}_s^{\text{hulp}}}\]
        \item Vermenigvuldig \belangrijk{alle} tussenresultaten met $\underline{K}$.
    \end{enumerate}
\end{theorie}

\begin{oefening}[Voorbeeld uit slides: Achterwaarts rekenen met schaalfactor]
    \textbf{Opgave:}
    ...
    \textbf{Oplossing:}
    \begin{enumerate}
        \item \textbf{Stel een fictieve hulpwaarde:} Kies $\underline{V}_2^{\text{hulp}} = 200\angle 0^\circ\text{ V}$ als referentie.
        \item \textbf{Takstromen achteraan:}
              \[\underline{I}_2^{\text{hulp}} = \frac{200\angle 0^\circ}{50} = 4\text{ A} \qquad,\qquad \underline{I}_L^{\text{hulp}} = \frac{200\angle 0^\circ}{j100} = -j2\text{ A}\]
        \item \textbf{Totale fictieve stroom:} $\underline{I}_{\text{tot}}^{\text{hulp}} = 4 - j2\text{ A} = 4{,}472\angle -26{,}57^\circ\text{ A}$.
        \item \textbf{Fictieve bronspanning:}
              \[\underline{V}_1^{\text{hulp}} = R_1 \cdot \underline{I}_{\text{tot}}^{\text{hulp}} = 4 \cdot (4 - j2) = 16 - j8\text{ V}\]
              \[\underline{V}_{\text{bron}}^{\text{hulp}} = \underline{V}_2^{\text{hulp}} + \underline{V}_1^{\text{hulp}} = 200 + (16 - j8) = 216 - j8\text{ V} = 216{,}15\angle -2{,}12^\circ\text{ V}\]
        \item \textbf{Schaalfactor $\underline{K}$:}
              \[\underline{K} = \frac{V_{\text{net}}\angle 0^\circ}{\underline{V}_{\text{bron}}^{\text{hulp}}} = \frac{230\angle 0^\circ}{216{,}15\angle -2{,}12^\circ} = 1{,}0641\angle +2{,}12^\circ\]
        \item \textbf{Werkelijke spanningen en stromen:}
              \[\underline{V}_2 = \underline{K} \cdot \underline{V}_2^{\text{hulp}} = 1{,}0641\angle 2{,}12^\circ \cdot 200\angle 0^\circ = \mathbf{212{,}8\angle 2{,}12^\circ\text{ V}}\]
              \[\underline{I}_{\text{bron}} = \underline{K} \cdot \underline{I}_{\text{tot}}^{\text{hulp}} = 1{,}0641\angle 2{,}12^\circ \cdot 4{,}472\angle -26{,}57^\circ = \mathbf{4{,}76\angle -24{,}45^\circ\text{ A}}\]
    \end{enumerate}
\end{oefening}
...
\begin{examenbox}[EXAMENVRAAG:]
    ...
    Omdat de keten \keyterm{lineair} is. Elk resultaat heeft de vorm
    $\underline{X} = \underline{H}\cdot\underline{V}_b$, waarbij $\underline{H}$ alleen van de
    impedanties afhangt. Vervang je $\underline{V}_b$ door $K\underline{V}_b$, dan wordt élke
    spanning en élke stroom met dezelfde factor $K$ vermenigvuldigd.

    Is $K$ reëel, dan verandert er niets aan de hoeken: het hele fasordiagramma wordt gewoon groter
    of kleiner. Is $K$ complex, dan draait het hele diagramma over $\arg(K)$, maar de
    \belangrijk{onderlinge} hoeken blijven gelijk. En die onderlinge hoeken zijn precies wat de
    arbeidsfactoren en de vermogens bepalen. Je hoeft dus nooit hoek per hoek te corrigeren.
\end{examenbox}

\begin{waarschuwing}[Wanneer je níet moet schalen]
    Staat de gegeven spanning \emph{binnenin} het net (bijvoorbeeld ``de spanning over de spoel is
    200~V'' of een voltmeteraflezing), dan is dat al je referentie. Reken gewoon van daaruit naar
    voor: \belangrijk{geen schaalstap nodig}. Kijk dus altijd eerst waar de gegeven waarde staat.
\end{waarschuwing}

\subsubsection{Methode 2: knooppuntanalyse (KCL)}

Gebruik dit zodra er \keyterm{meerdere bronnen} zijn, of wanneer het net geen zuivere
...
\begin{theorie}[Werkwijze]
    \begin{enumerate}
        ...
        \item Kies één knoop als \keyterm{aarde}. Kies bij voorkeur de knoop waar de meeste takken
...
Wanneer een lineaire keten meerdere onafhankelijke bronnen bevat met \belangrijk{verschillende frequenties}
...
\begin{oefening}[Voorbeeld uit slides: Superpositie met DC en meerdere AC-frequenties]
    ...
    \end{itemize}
    geschakeld over weerstand $R = 4\,\Omega$, spoel $L = 1\text{ H}$ en condensator $C = 0{,}1\text{ F}$.

    \textbf{Oplossing:}
    \begin{enumerate}
        \item \textbf{Deelrespons DC ($\omega_1 = 0$):}
              Op DC is de condensator een open keten ($Z_C = \infty$) en de spoel een kortsluiting ($Z_L = 0$).
              Uit spanningsdeling: $V_{o,1} = 1\text{ V}$.
        \item \textbf{Deelrespons $v_2(t)$ ($\omega_2 = 2\text{ rad/s}$):}
              $\underline{V}_2 = 10\angle 0^\circ\text{ V}$, $\underline{Z}_L = j(2)(1) = j2\,\Omega$, $\underline{Z}_C = \frac{1}{j(2)(0{,}1)} = -j5\,\Omega$.
        ...
        \item \textbf{Deelrespons $i_3(t)$ ($\omega_3 = 5\text{ rad/s}$):}
              ...
              $\underline{Z}_L = j5\,\Omega$, $\underline{Z}_C = -j2\,\Omega$.
...
{Voor maximale overdracht van \keyterm{actief vermogen} aan een wisselstroombelasting $\underline{Z}_L = R_L + jX_L$ moet de belastingsimpedantie de \keyterm{complex geconjugeerde} zijn van de Thévenin-impedantie $\underline{Z}_{\text{th}} = R_{\text{th}} + jX_{\text{th}}$.
    De reactanties heffen elkaar dan op ($X_L = -X_{\text{th}}$), waardoor totale resonantie optreedt.
    \newline
    \textbf{Speciaal geval (zuiver ohmse belasting $R_L$):} Indien $X_L = 0$, treedt het maximum op wanneer $R_L = |\underline{Z}_{\text{th}}| = \sqrt{R_{\text{th}}^2 + X_{\text{th}}^2}$.}

\subsection{Werken met enkel de grootte van stromen}

Soms geeft het examen je alleen \keyterm{ampèremeteraflezingen}: drie stroomgrootten zonder
...
dan vormen hun fasoren een \belangrijk{gesloten driehoek}. Met de cosinusregel haal je de
...
\keyterm{Hoe weet je welk teken?} Een condensatortak laat de stroom \textbf{voorijlen} (hoek
...
\begin{itemize}
    \item \keyterm{Spanningen teken je kop-staart.} Begin in een knoop, teken de spanningsval over
          ...
          gevraagd.
    \item \keyterm{Stromen teken je allemaal vanuit één punt}, als een waaier.
\end{itemize}

\begin{waarschuwing}[Vermeld je schaal]
    Gebruik \belangrijk{twee verschillende schalen}, één voor spanningen en één voor stromen
    (bijvoorbeeld $1:40$ voor V en $1:2$ voor A), en \textbf{schrijf ze bij het diagramma}.
    Zonder schaal is een diagramma niet controleerbaar en verlies je punten. Neem je geodriehoek en
    passer mee: het moet \emph{op schaal} zijn, niet uit de losse hand.
\end{waarschuwing}
...
\begin{conceptbox}[Waarom gebruiken we 3-fasige systemen?]
    ...
    \begin{enumerate}
        ...
        \item \textbf{Drastische koper- en kabelbesparing:} Om hetzelfde totale vermogen te transporteren over
              ...
              net tot \belangrijk{$25\%$ à $50\%$ minder geleidermateriaal} nodig dan equivalente 1-fase leidingen.
...
resultaat precies \belangrijk{in het midden} tussen beide --- dus $60^\circ$ van elk. Ten
...
Zijn de belastingen gelijk, dan loopt er \belangrijk{geen circulatiestroom} rond in de driehoek
...
\begin{waarschuwing}[De 30 graden zijn geen detail]
    De factor $\sqrt{3}$ onthoudt iedereen, de \belangrijk{hoekverschuiving van $30^\circ$} niet.
    Vergeet je die, dan zit je $30^\circ$ fout op je fasehoek, en dus op je arbeidsfactor en je
    vermogens.

    \keyterm{Waar komt die $30^\circ$ vandaan?} De lijnstroom is het \emph{verschil} van twee
...
{Met $Z_a, Z_b, Z_c$ de sterimpedanties in $\Omega$ en $Z_1, Z_2, Z_3$ de driehoekimpedanties in
    $\Omega$. Ezelsbruggetje: in de teller staat het \keyterm{product van de twee driehoektakken die
        aan dat sterpunt raken}, in de noemer de \keyterm{som van alle drie}.}
...
{Met dezelfde symbolen. De teller is telkens \emph{dezelfde} som; enkel de noemer wisselt: de
    sterimpedantie die \keyterm{tegenover} die driehoektak ligt.}

\begin{examenbox}
    Is de belasting \belangrijk{evenwichtig}, dan vereenvoudigen beide formules tot
    \[Z_\Delta = 3 Z_Y\]
...
\begin{waarschuwing}[Alleen $P_1+P_2$ geldt altijd]
    $P_{totaal} = P_1 + P_2$ volgt \belangrijk{enkel uit KCL} en geldt dus altijd: evenwichtig of
    niet, ster of driehoek.

    De formules voor $Q_{totaal}$, $\theta$ en $S$ hieronder gaan er daarentegen van uit dat de drie
    fasen \belangrijk{identiek} zijn. Bij een \emph{ongebalanceerde} belasting zijn ze gewoon
...
Een wattmeter heeft twee spoelen: een \keyterm{stroomspoel} in serie met de lijn, en een
\keyterm{spanningsspoel} tussen die lijn en de referentielijn. Hij geeft het actief vermogen dat bij
...
\begin{waarschuwing}[De klassiekste fout van het hele vak]
    ...
    dan is $\underline{V}_{cb} = V_L\angle 60^\circ$ en \belangrijk{niet} $V_L\angle -120^\circ$.
    Elk jaar sneuvelen hier punten op.
\end{waarschuwing}

\keyterm{Waarom volstaan twee wattmeters?} Zonder nulgeleider geldt
...
\belangrijk{Maar:} één wattmeter meet dan níet het vermogen van één fase. Alleen de \emph{som} heeft
...
\begin{theorie}[Interpretatie van wattmeteraflezingen bij evenwichtig net]
    ...
    \begin{itemize}
        ...
        \item $\phi > 60^\circ$ ($\cos\phi < 0{,}5$): $W_1 > 0$ en \belangrijk{$W_2 < 0$} (meter 2 slaat negatief uit / wijzer slaat terug). Om de waarde af te lezen moet men de spanningsspoel ompolen; $P_{\text{totaal}}$ blijft algebraïsch $W_1 + W_2$.
...
\begin{center}
    \begin{tabular}{@{}p{62mm}p{20mm}p{28mm}@{}}
        ...
        \midrule
        Eén fase uitrekenen en $\pm120^\circ$ draaien    & ja                   & \belangrijk{nee}        \\
        $I_L = \sqrt{3}\,I_f\angle -30^\circ$ (driehoek) & ja                   & \belangrijk{nee}        \\
        $\underline{Z}_\Delta = 3\underline{Z}_Y$        & ja                   & \belangrijk{nee}        \\
        $\underline{I}_N = 0$ in de nulgeleider          & ja                   & \belangrijk{nee}        \\
        $Q_{tot} = \sqrt{3}(W_2 - W_1)$                  & ja                   & \belangrijk{nee}        \\
        $P_{tot} = W_1 + W_2$                            & ja                   & \keyterm{ja}            \\
...
{Bij een \emph{ongebalanceerde sterbelasting zonder nulgeleider} verschuift het zwevende sterpunt $N'$
    ten opzichte van het nulpunt van de bron $N$. Hierin zijn $\underline{Y}_a = 1/\underline{Z}_a$,
    $\underline{Y}_b = 1/\underline{Z}_b$, $\underline{Y}_c = 1/\underline{Z}_c$ de takadmittanties.
...
\begin{examenbox}
    ...
    takken plots de volle \belangrijk{lijn}spanning in plaats van de fasespanning. Dat is
    $\sqrt{3}$ keer meer spanning en dus \belangrijk{3 keer meer vermogen} per tak.
\end{examenbox}
...
\begin{examenbox}[EXAMENTIP: Belangrijke theorievragen]
    Op het theorie-examen handelt er doorgaans minstens één vraag over elektrische veiligheid (de 5 vitale veiligheidsregels, de MCB-karakteristiek, de werking van de RCD/somstroomtransformator, overbelasting vs.\ kortsluiting, of aanraakspanning).

    De theorie is opgebouwd rond vier pijlers:
    \keyterm{Gevaren} $\rightarrow$ \keyterm{Veilig werken} $\rightarrow$ \keyterm{Beveiliging tegen overstroom} $\rightarrow$ \keyterm{Bescherming tegen elektrische schokken}.
\end{examenbox}

\subsection{Elektrische gevaren}

Elektriciteit brengt twee fundamentele gevaren met zich mee: risico's voor \keyterm{installaties}
(brand, smelten, mechanische vernieling) én voor \keyterm{mensen} (elektrocutie, brandwonden, vallen).
...
\belangrijk{evenredig met het kwadraat van de piekstroom ($i_{\text{piek}}^2$)}. Hierdoor kunnen
...
\begin{examenbox}[EXAMENVRAAG:]
    ...
    Om de enorme \keyterm{elektrodynamische krachten} bij kortsluiting op te vangen. De krachten
...
\begin{enumerate}
    \item \keyterm{Stroomsterkte $I_B$} --- hoe groter de stroom, hoe zwaarder het effect.
    \item \keyterm{Lichaamsimpedantie $Z_B$} --- bepaalt bij een gegeven spanning de stroom
          ($I_B = V/Z_B$). Droge eeltlaag heeft een hoge weerstand ($>2000\,\Omega$), natte of
          beschadigde huid zakt naar $<500-1000\,\Omega$.
    \item \keyterm{Frequentie $f$} --- netfrequentie ($50-60\text{ Hz}$) is fysiologisch het
    ...
    \item \keyterm{Blootstellingsduur $t$} --- hoe langer de stroom vloeit, hoe groter de kans dat
          de kwetsbare hartfase geraakt wordt en hoe meer warmte gedissipeerd wordt.
    \item \keyterm{Aanraakspanning $V$} --- drijft de stroom door het lichaam en doorbreekt bij
          hogere waarden de huidbarrière.
    \item \keyterm{Stroombaan door het lichaam} --- trajecten over het hart (bv.\ linkerhand naar
...
\begin{waarschuwing}[Niet de spanning maar de stroom doodt!]
    De fysiologische schade hangt direct af van de \belangrijk{stroom $I_B$} door het lichaam en de
...
\begin{itemize}
    \item \keyterm{Zone AC-1 ($I_B < 0{,}5\text{ mA}$):} onder de waarnemingsdrempel (curve a). Geen merkbaar effect.
    \item \keyterm{Zone AC-2 ($0{,}5\text{ mA} \le I_B \le \text{curve b}$):} waarneembaar, lichte spiertinteling,
          maar geen schadelijke fysiologische gevolgen.
    \item \keyterm{Zone AC-3 (curve b tot $c_1$):} spierverkramping, ademhalingsmoeilijkheden, loslaten niet meer
          mogelijk. Effecten zijn \emph{omkeerbaar} zodra de stroom onderbroken wordt.
    \item \keyterm{Zone AC-4 (boven curve $c_1$):} levensgevaar door \belangrijk{ventrikelfibrillatie}, hartstilstand
...
\begin{examenbox}[EXAMENVRAAG:]
    ...
    \textbf{Assen:} Horizontaal de \keyterm{lichaamsstroom $I_B$} in mA (logaritmisch), verticaal de
    \keyterm{duur $t$} in ms (logaritmisch).
    ...
    kwetsbare T-golf van de hartslag valt. \keyterm{Precies daarom biedt een snelle RCD ($<30\text{ ms}$)
...
lichaamsweerstand $Z_B$ de stroomgrens omgerekend naar een \keyterm{veilige conventionele aanraakspanningsgrens} ($U_L$):
...
\begin{itemize}
    \item \keyterm{Rechtstreekse aanraking (Direct contact):} een persoon raakt een actieve geleider aan die
    ...
    \item \keyterm{Onrechtstreekse aanraking (Indirect contact):} een persoon raakt een metalen
...
\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}[scale=0.92, line cap=round, line join=round]
        % Transformator zijde (links)
        \draw[thick, schoolBlue!80!black, fill=schoolBlue!10] (-0.3, 0.4) rectangle (1.7, 3.4);
        \node[font=\bfseries\small, text=schoolBlue!90!black, align=center] at (0.7, 2.7) {Voedingstrafo\\$U_0 = 230\text{ V}$};
        
        \node[circle, fill=schoolBlue!90!black, inner sep=1.5pt, label=above:{\small L1}] (L_src) at (1.7, 1.9) {};
        \node[circle, fill=black, inner sep=1.5pt, label=below:{\small N}] (N_src) at (1.7, 0.8) {};
        
        % Nulaarding trafo R_N
        \draw[thick] (N_src) -- (2.3, 0.8) -- (2.3, -0.4);
        \draw[thick, fill=white] (2.0, -0.4) rectangle (2.6, -1.1);
        \node[font=\footnotesize] at (2.3, -0.75) {$R_N$};
        \draw[thick] (2.3, -1.1) -- (2.3, -1.5);
        \draw[thick] (1.9, -1.5) -- (2.7, -1.5);
        \draw[thick] (2.05, -1.65) -- (2.55, -1.65);
        \draw[thick] (2.2, -1.8) -- (2.4, -1.8);
        \node[font=\scriptsize, text=gray!80!black, below] at (2.3, -1.85) {Trafo-aarde ($R_N = 10\,\Omega$)};
        
        % Leidingen naar verbruiker
        \draw[thick, schoolBlue] (L_src) -- (6.0, 1.9);
        \draw[thick, black] (N_src) -- (6.0, 0.8);
        
        % Verbruiker (rechts)
        \draw[thick, fill=white] (6.0, 0.5) rectangle (7.4, 2.2);
        \node[font=\small, align=center] at (6.7, 1.35) {Interne\\belasting};
        
        % Metalen behuizing (gestel)
        \draw[very thick, schoolRed, dashed] (5.5, 0.2) rectangle (7.9, 2.6);
        \node[font=\bfseries\footnotesize, text=schoolRed, anchor=south] at (6.7, 2.65) {Metalen gestel (behuizing)};
        
        % Isolatiefout
        \draw[->, ultra thick, schoolRed] (5.8, 1.9) -- (5.8, 2.6) -- (5.5, 2.6);
        \node[font=\scriptsize\bfseries, text=schoolRed, above] at (5.5, 2.8) {Isolatiefout ($R_f \approx 0\,\Omega$)};
        
        % Aardingsgeleider PE van behuizing naar R_A
        \draw[thick, schoolGreen!80!black] (7.5, 0.2) -- (7.5, -0.4);
        \draw[thick, fill=white] (7.2, -0.4) rectangle (7.8, -1.1);
        \node[font=\footnotesize] at (7.5, -0.75) {$R_A$};
        \draw[thick, schoolGreen!80!black] (7.5, -1.1) -- (7.5, -1.5);
        \draw[thick] (7.1, -1.5) -- (7.9, -1.5);
        \draw[thick] (7.25, -1.65) -- (7.75, -1.65);
        \draw[thick] (7.4, -1.8) -- (7.6, -1.8);
        \node[font=\scriptsize, text=gray!80!black, below] at (7.5, -1.85) {Gebruiker-aarde ($R_A = 30\,\Omega$)};
        
        % Persoon die behuizing aanraakt
        \draw[thick, schoolOrange!90!black] (9.0, 1.9) circle (0.22); % hoofd
        \draw[thick, schoolOrange!90!black] (9.0, 1.68) -- (9.0, 0.8); % romp
        \draw[thick, schoolOrange!90!black] (9.0, 1.45) -- (7.9, 1.7); % arm naar behuizing
        \draw[thick, schoolOrange!90!black] (9.0, 0.8) -- (8.7, 0.0); % been L
        \draw[thick, schoolOrange!90!black] (9.0, 0.8) -- (9.3, 0.0); % been R
        
        % Lichaamsweerstand R_B
        \draw[thick, fill=white] (8.7, 0.0) rectangle (9.3, -0.7);
        \node[font=\footnotesize] at (9.0, -0.35) {$R_B$};
        \draw[thick] (9.0, -0.7) -- (9.0, -1.5);
        \draw[thick] (8.6, -1.5) -- (9.4, -1.5);
        \draw[thick] (8.75, -1.65) -- (9.25, -1.65);
        \draw[thick] (8.9, -1.8) -- (9.1, -1.8);
        \node[font=\scriptsize, text=gray!80!black, below] at (9.0, -1.85) {Voeten ($R_B = 1000\,\Omega$)};
        
        % Foutstroomlus aanduiding
        \draw[->, dashed, ultra thick, schoolRed] (3.6, 2.1) -- (4.8, 2.1) node[midway, above, font=\scriptsize\bfseries] {$I_f$};
        \draw[->, dashed, thick, schoolOrange!90!black] (8.4, 1.6) -- (8.8, 1.6) node[midway, above, font=\tiny\bfseries] {$I_B$};
        
        % Aanraakspanning pijl
        \draw[<->, thick, schoolBlue] (10.3, 2.5) -- (10.3, -1.5) 
            node[midway, right, font=\footnotesize\bfseries, align=left, text=schoolBlue] {Aanraakspanning:\\$U_T = I_f \cdot R_A$};
    \end{tikzpicture}
    \caption{Elektrisch vervangingsschema van onrechtstreekse aanraking (indirect contact) in een TT-netwerk. De foutstroom $I_f$ vloeit via het metalen gestel en de aardverspreidingsweerstand $R_A$ terug naar de transformatoraarding $R_N$.}
...
\begin{oefening}[Waarom een MCB niet volstaat en een RCD strikt noodzakelijk is]
    ...
    \begin{enumerate}
        ...
        \item \textbf{Reactie van de RCD ($30\text{ mA}$):}
              Omdat $I_f = 5{,}75\text{ A} \gg 30\text{ mA}$, schakelt de RCD \belangrijk{onmiddellijk uit} ($<30\text{ ms}$), waardoor de spanning wegvalt vóór ventrikelfibrillatie kan optreden.
    \end{enumerate}
\end{oefening}
...
\belangrijk{strikte volgorde} worden toegepast:

\begin{examenbox}[EXAMENVRAAG: De 5 Gouden Regels]
    \begin{enumerate}
        \item \keyterm{Volledig uitschakelen en scheiden (Disconnect):} alle polen en alle mogelijke
              voedingsbronnen (inclusief noodstroomaggregaten, PV-omvormers en condensatorbatterijen) scheiden.
        \item \keyterm{Beveiligen tegen herinschakeling (Lockout/Tagout):} schakelaars mechanisch vergrendelen
              met een hangslot en voorzien van een waarschuwingslabel.
        \item \keyterm{Spanningsloosheid controleren (Verify zero voltage):} meten met een goedgekeurde
              tweepolige spanningsaanwijzer. \belangrijk{Controleer het meettoestel altijd vlak vóór én vlak
                  na de meting op een gekende spanningsbron!}
        \item \keyterm{Aarden en kortsluiten (Earth and short-circuit):} de geleiders onderling en met de aarde
              verbinden (verplicht bij hoogspanning, aanbevolen bij laagspanning) om inductieve en capacitieve
              restspanningen af te voeren.
        \item \keyterm{Afschermen en afbakenen (Protect and delineate):} nabijgelegen delen die nog onder
              spanning staan fysiek afschermen met isolerende doeken of schotten en de werkzone markeren.
    \end{enumerate}
\end{examenbox}
...
\begin{itemize}
    \item \keyterm{Europese Laagspanningsrichtlijn (LVD 2014/35/EU):} legt fundamentele veiligheidseisen
          op voor elektrisch materiaal tussen $50-1000\text{ V}$ AC en $75-1500\text{ V}$ DC.
    \item \keyterm{AREI (Algemeen Reglement op de Elektrische Installaties):} het bindende Belgische
          wetboek voor veilige installaties.
    \item \keyterm{Norminstituten:} \textbf{IEC} (mondiaal), \textbf{CENELEC} (Europees) en \textbf{CEBEC/NBN} (België).
\end{itemize}
...
\begin{center}
    \begin{tabular}{@{}p{24mm}p{48mm}p{50mm}@{}}
        ...
        \textbf{Detectie}      & \keyterm{Thermisch} (bimetaal dat opwarmt).                         & \keyterm{Magnetisch} (elektromagnetische spoel).                             \\[1mm]
        \textbf{Uitschakeling} & \belangrijk{Vertraagd} (invers-tijd: seconden tot minuten).         & \belangrijk{Quasi ogenblikkelijk} ($<10-20\text{ ms}$).                      \\
        \bottomrule
    \end{tabular}
\end{center}
...
\begin{examenbox}[EXAMENVRAAG: MCB-karakteristiek]
    ...
    \textbf{Assen:} Horizontaal de \keyterm{stroom als veelvoud van $I_n$} ($I/I_n$, logaritmisch),
    verticaal de \keyterm{uitschakeltijd $t$} in seconden (logaritmisch).
    ...
    \begin{itemize}
        \item \keyterm{Type B ($3 - 5 \times I_n$):} voor kringen zonder inschakelpieken (elektrische
              verwarming, lange leidingen met lage kortsluitstroom).
        \item \keyterm{Type C ($5 - 10 \times I_n$):} de universele standaard voor huishoudelijke en
              algemene installaties (verlichting, stopcontacten, kleine motoren).
        \item \keyterm{Type D ($10 - 20 \times I_n$):} voor zware inductieve lasten met hoge
              inschakelpieken (transformatoren, grote motoren, lasapparaten).
        \item \keyterm{Type MA ($12{,}5 \times I_n$):} puur magnetisch (geen bimetaal); dient enkel
...
\begin{itemize}
    \item \keyterm{Aardlus:} een blanke koperen lus onder de funderingsmuren (minstens $60\text{ cm}$
          diep) bij nieuwbouw. Bij renovatie gebruikt men \keyterm{aardingspennen} (koperen staven) in de grond.
    \item \keyterm{Aardingsscheider (meetklem):} onderbreekt de koppeling met het netwerk om de
    ...
    \item \keyterm{Hoofdequipotentiaalverbinding (HEB):} verbindt de hoofdaardklem met alle metalen
          hoofdleidingen van het gebouw (waterleiding, gasleiding, CV-buizen, stalen bouwstructuur).
          Hierdoor kunnen er tussen deze leidingen onderling \belangrijk{geen gevaarlijke potentiaalverschillen} ontstaan.
    \item \keyterm{Bijkomende equipotentiaalverbinding:} verplicht in vochtige ruimtes (badkamer) tussen
...
\begin{examenbox}[EXAMENVRAAG: RCD / Aardlekschakelaar]
    \textit{Wat is een RCD? Waartegen beschermt het? Geef alle synoniemen en leg het werkingsprincipe uit.}

    \textbf{Definitie:} Een \keyterm{RCD} (Residual Current Device) is een automatisch beveiligingstoestel
    dat de installatie uitschakelt zodra er een lekstroom (differentiaalstroom $I_\Delta$) naar de
    aarde vloeit.

    \textbf{Synoniemen:} Aardlekschakelaar, Differentieelschakelaar, Verliesstroomschakelaar, RCCB, RCD.

    \textbf{Beschermingsdoel:}
    \begin{itemize}
        \item \belangrijk{Persoonsbeveiliging ($30\text{ mA}$):} bescherming tegen elektrocutie bij directe en onrechtstreekse aanraking.
        \item \belangrijk{Brandbeveiliging ($300\text{ mA}$):} voorkomt dat isolatiefouten ontbranden door Joule-opwarming ($P = I_\Delta^2 R$).
    \end{itemize}
\end{examenbox}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=0.90, line cap=round, line join=round]
        % Torus (ringkern)
        \draw[fill=gray!20, draw=gray!70, double, double distance=15pt, line width=1pt] (5.0, 2.0) circle (1.85);
        \node[font=\bfseries\small, text=gray!80!black, align=center] at (5.0, 2.0) {Ferromagnetische\\Torus (Ringkern)};
        
        % Voedingsklemmen
        \node[font=\bfseries\small, text=schoolBlue] at (-0.2, 3.2) {Fase L1};
        \node[font=\bfseries\small, text=black] at (-0.2, 0.8) {Nul N};
        
        % Verbruiker
        \draw[thick, fill=schoolBlue!10, draw=schoolBlue] (9.0, 0.4) rectangle (10.8, 3.6);
        \node[font=\bfseries\small, align=center, text=schoolBlue!90!black] at (9.9, 2.0) {Verbruiker\\(Belasting)};
        
        % Fase L1 lijn
        \draw[thick, schoolBlue] (0.6, 3.2) -- (1.8, 3.2);
        \draw[thick, schoolBlue] (1.8, 3.2) -- (2.2, 3.55); % schakelcontact
        \draw[thick, schoolBlue] (2.5, 3.2) -- (9.0, 3.2);
        \draw[fill=schoolBlue] (1.8, 3.2) circle (0.05);
        \draw[fill=schoolBlue] (2.5, 3.2) circle (0.05);
        \draw[->, ultra thick, schoolBlue] (3.4, 3.4) -- (4.2, 3.4) node[above, font=\scriptsize\bfseries] {$i_L(t)$};
        
        % Nul N lijn
        \draw[thick, black] (0.6, 0.8) -- (1.8, 0.8);
        \draw[thick, black] (1.8, 0.8) -- (2.2, 1.15); % schakelcontact
        \draw[thick, black] (2.5, 0.8) -- (9.0, 0.8);
        \draw[fill=black] (1.8, 0.8) circle (0.05);
        \draw[fill=black] (2.5, 0.8) circle (0.05);
        \draw[<-, ultra thick, black] (3.4, 0.6) -- (4.2, 0.6) node[below, font=\scriptsize\bfseries] {$i_N(t)$};
        
        % Secundaire meetspoel
        \draw[thick, schoolRed] (4.6, 0.15) arc (180:360:0.4);
        \draw[thick, schoolRed] (4.5, 0.05) arc (180:360:0.5);
        \draw[thick, schoolRed] (4.5, 0.05) -- (4.5, -1.2) -- (4.8, -1.2);
        \draw[thick, schoolRed] (5.5, 0.05) -- (5.5, -1.2) -- (5.2, -1.2);
        
        % Uitschakelrelais
        \draw[fill=schoolRed!15, draw=schoolRed, thick] (4.2, -1.7) rectangle (5.8, -1.1);
        \node[font=\scriptsize\bfseries, text=schoolRed, align=center] at (5.0, -1.4) {Uitschakel-\\relais};
        \draw[dashed, ultra thick, schoolRed, ->] (5.0, -1.1) -- (5.0, -0.4) -- (2.15, -0.4) -- (2.15, 3.1);
        \node[font=\tiny\bfseries, text=schoolRed, right] at (2.2, 1.9) {Mechanische ontgrendeling ($<30\text{ ms}$)};
        
        % Testcircuit
        \draw[thick, schoolGreen!80!black] (1.2, 3.2) -- (1.2, 4.4) -- (3.8, 4.4);
        \draw[thick, schoolGreen!80!black] (3.8, 4.6) -- (4.4, 4.6); % knop T
        \draw[thick, schoolGreen!80!black] (4.1, 4.6) -- (4.1, 4.8);
        \draw[fill=schoolGreen!80!black] (3.8, 4.4) circle (0.05);
        \draw[fill=schoolGreen!80!black] (4.4, 4.4) circle (0.05);
        \node[font=\scriptsize\bfseries, text=schoolGreen!80!black, above] at (4.1, 4.8) {Testknop $T$};
        \draw[thick, schoolGreen!80!black] (4.4, 4.4) -- (5.8, 4.4);
        \draw[fill=white, draw=schoolGreen!80!black, thick] (5.8, 4.2) rectangle (6.8, 4.6);
        \node[font=\scriptsize\bfseries, text=schoolGreen!80!black] at (6.3, 4.4) {$R_T$};
        \draw[thick, schoolGreen!80!black] (6.8, 4.4) -- (8.0, 4.4) -- (8.0, 0.8);
        \draw[fill=schoolGreen!80!black] (8.0, 0.8) circle (0.05);
        \draw[fill=schoolGreen!80!black] (1.2, 3.2) circle (0.05);
        \node[font=\tiny, text=schoolGreen!80!black, align=center] at (6.3, 4.85) {Teststroom $I_T = U/R_T$ loopt buiten de torus om};
    \end{tikzpicture}
    \caption{Elektrisch schema en werkingsprincipe van een aardlekschakelaar (RCD). De testknop $T$ met serieweerstand $R_T$ leidt een stroom buiten de torus om om de uitschakeling te testen.}
    \label{fig:rcd-principe-tikz}
\end{figure}
\FloatBarrier

\frm{Fluxbalans in de RCD-ringkern}{\Phi_{\text{net}}(t) = \frac{\mu A}{l} \cdot N_1 \left( i_L(t) - i_N(t) \right) = \frac{\mu A}{l} \cdot N_1 i_\Delta(t)}
{Bij een foutloze kring is $i_L(t) = i_N(t) \implies \Phi_{\text{net}} = 0$. Bij een lekstroom $i_\Delta(t) \neq 0$ ontstaat een netto wisselende flux $\Phi_{\text{net}}(t)$.}

\frm{Geïnduceerde spanning in de meetspoel}{e_{\text{sec}}(t) = -N_{\text{sec}} \frac{d\Phi_{\text{net}}}{dt} \qquad \Longrightarrow \qquad \text{Relais tript bij } I_\Delta \ge I_{\Delta n}}
{Met $N_{\text{sec}}$ het aantal windingen van de secundaire meetspoel en $e_{\text{sec}}$ de opgewekte EMK die het uitschakelrelais bekrachtigt.}
...
\begin{itemize}
    ...
    \item De uitschakeling gebeurt hierdoor \belangrijk{ruim vóór curve $c_1$}, waardoor de kans op dodelijke
          ventrikelfibrillatie (zone AC-4) zo goed als uitgesloten is.
\end{itemize}

\textbf{Types RCD volgens aard van de foutstroom:}
\begin{itemize}
    \item \keyterm{Type AC:} reageert enkel op zuivere sinusvormige wisselstromen (verouderd).
    \item \keyterm{Type A:} reageert op sinusvormige wisselstromen én \textbf{pulserende gelijkstromen} (residentiële standaard volgens AREI).
    \item \keyterm{Type B:} reageert op AC, pulserende DC én \textbf{gladde gelijkstromen} (PV-omvormers, laadpalen, frequentieregelaars).
\end{itemize}
...
\begin{waarschuwing}[Essentiële beperkingen van een RCD (Waar een RCD níet tegen beschermt)]
    \begin{enumerate}
        ...
        \item Een RCD reageert \belangrijk{niet} wanneer een persoon gelijktijdig de fase én de nulgeleider aanraakt
...
\begin{oefenblok}[Optellen van een sinus en een cosinus]
    ...
    \begin{waarschuwing}[Dit is de valstrik]
        Een sinus en een cosinus mag je \belangrijk{niet} zomaar als fasor optellen. Zet ze eerst
...
\begin{oefenblok}[Zelfinductie uit twee spanningsmetingen]
    ...
    \belangrijk{73\% te veel}. Zodra er een spoel of condensator in het spel is, tellen spanningen
    nooit gewoon op.
\end{oefenblok}
...
\begin{oefenblok}[Reactantie en teken uit de arbeidsfactor]
    ...
    \keyterm{Capacitief} betekent dat de stroom voorijlt op de spanning, dus de fasehoek is negatief:
...
\begin{oefenblok}[Van fasordiagramma naar vermogen]
    ...
    Dat is de hoek \keyterm{van $V$ naar $I$}:
    \[\phi = 25^\circ - (-40^\circ) = 65^\circ\]

    \textbf{Stap 2: het ogenblikkelijk vermogen is het product van de tijdsfuncties.}
    Let op: hier gebruik je \belangrijk{piekwaarden}, niet de effectieve waarden.
    ...
    Het eerste stuk is \keyterm{constant}, het tweede oscilleert met $2\omega$ en heeft gemiddelde nul.
...
\begin{oefenblok}[Arbeidsfactorcorrectie]
    ...
    \keyterm{Dat is de hele bedoeling van arbeidsfactorcorrectie:} minder stroom door de
...
\begin{oefenblok}[Arbeidsfactor van twee belastingen samen]
    ...
    \begin{waarschuwing}[Twee klassieke fouten]
        ...
        Schijnbare vermogens zijn \belangrijk{vectoren}: je mag ze alleen complex optellen.
...
\begin{oefenblok}[Knooppuntanalyse met twee bronnen]
    ...
    Er blijven twee onbekende knooppunten over ($A$ en $B$), dus je hebt \keyterm{twee}
...
\begin{oefenblok}[Cosinusregel wanneer je enkel stroomgrootten kent]
    ...
    \begin{examenbox}
        ...
        \keyterm{cosinusregel} de sleutel. De drie fasoren vormen dan een gesloten driehoek.
    \end{examenbox}
    ...
    \keyterm{Waarom ligt $\underline{I}_C$ boven en $\underline{I}_L$ onder?} Een condensator laat de
    ...
    \begin{waarschuwing}[Doe dit niet met een stelsel]
        ...
        \belangrijk{Eén enkele complexe deling volstaat.}
    \end{waarschuwing}
    ...
    \keyterm{Waarom capacitief?} $|I_C| = 40$~A is veel groter dan $|I_L| = 17{,}9$~A: de condensator
...
\begin{oefenblok}[Ongebalanceerde driehoek met twee wattmeters]
    ...
    \begin{waarschuwing}[Ongebalanceerd]
        De belasting is ongebalanceerd, dus je mag \belangrijk{niet} één fase uitrekenen en
    ...
    Lijn $b$ is dus het \keyterm{gemeenschappelijke referentiepunt}: daar staat géén wattmeter.
    ...
    \begin{waarschuwing}[Klassieke tekenfout]
        $\underline{V}_{cb} = -\underline{V}_{bc} = 208\angle 60^\circ$, \belangrijk{niet}
    ...
    informatie over het totale actief vermogen in, \keyterm{ook bij onevenwicht}, en ook als de last
    in ster of in driehoek staat. Wél geldt dan: één wattmeter meet níet het vermogen van één fase,
    alleen de som heeft betekenis. En $Q_{tot} = \sqrt{3}(W_2 - W_1)$ geldt \belangrijk{enkel bij
...
\begin{oefenblok}[Evenwichtige ster met één extra last ertussen]
    ...
    \begin{examenbox}
        \keyterm{Kernidee:} dit net is de \textbf{superpositie} van een evenwichtige sterlast en één
    ...
    \keyterm{De condensatortak heeft de arbeidsfactor verbeterd} van $0{,}906$ naar $0{,}94$ --- dat
