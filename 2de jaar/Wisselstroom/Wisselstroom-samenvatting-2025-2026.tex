...
\section{Cursusinfo}
\subsection{Structuur 2024-2025}
...
Je kunt die allemaal doen met je niet grafische rekenmachine door de mode op
\textbf{complex} $a + jb$ of \textbf{polar} $A_m \angle \omega$ te zetten.
...
\begin{oefening}[Gemiddelde en effectieve stroom]
    ...
    Een bron van $110\,\text{V}$ (effectieve waarde) voedt een reeks van een diode en een weerstand
    van $50\,\Omega$. De diode laat enkel de positieve helft van de sinus door (enkelzijdige
...
ELI the ICE man; L is de spoel, C is de condensator, I is de stroom, E is de spanning. Hiermee kun je zien,
afhankelijk van de belasting, of de stroom leading of lagging is. Als de stroom voorloopt op de spanning spreken we van een \textbf{leading} stroom.
...
\frm{Reactantie condensator}{X_C = \frac{-1}{\omega C}}{Met $X_C$ de capacitieve reactantie in Ohm, $\omega$ de hoeksnelheid in rad/s en C de capaciteit in Farad.}
...
\frm{Hoek van de impedantie}{\tan \phi = \frac{X_L + X_C}{R}}{Met $\phi$ de fasehoek van de impedantie in radialen, R de weerstand in Ohm, $X_L$ de inductieve reactantie in Ohm en $X_C$ de capacitieve reactantie in Ohm.}
...
We willen dit uitdrukken in de rms-waarden van spanning en stroom in plaats van in de piekwaarden $V_m$ en $I_m$.
...
Het wil vooral tonen dat netwerkanalyse (zie elektrische netwerken) ook kan toegepast worden op
AC netwerken.
...
Er zijn twee werkwijzen, en de keuze hangt af van het aantal bronnen.
...
\begin{minipage}{0.45\textwidth}
    ...
    met $V_{f1}$-$V_{f2}$ =>$120^\circ$. $V_L$ is dus $120^\circ$ gedraaid.
\end{minipage}
\begin{minipage}{0.5\textwidth}
    \centering
    \includegraphics[width=0.8\textwidth]{assets/fasors.png}
\end{minipage}
...
\newline
Nog een laatste: Je hebt een 3-fase stersysteem met een lijnspanning van $400 V$. Je hebt een fasestroom van $5 A<30^\circ$.
...
Als de belastingen gelijk zijn, zijn er geen terugstromingen door de bronnen.
...
\begin{examenbox}
    Dit hoofdstuk is \textbf{gegarandeerde examenstof}: van de vijf korte theorievragen gaat er
    bijna altijd één over elektrische veiligheid. De vragen die het vaakst terugkomen staan
    hieronder telkens in een kader.
\end{examenbox}

De slides delen dit hoofdstuk op in vier stukken, en die volgorde houden we hier aan:
gevaren $\rightarrow$ veilig werken $\rightarrow$ bescherming tegen overstroom $\rightarrow$
bescherming tegen elektrische schokken.

\subsection{Elektrische gevaren}

Elektriciteit is gevaarlijk voor \keyterm{installaties} (brand) én voor \keyterm{mensen}
(ongevallen). Die twee vragen een verschillende aanpak, dus we bekijken ze apart.

\subsubsection{Gevaren voor installaties}

\textbf{Thermische gevaren.} Elke geleider heeft een weerstand, dus bij stroomdoorgang ontwikkelt
hij warmte volgens $P = I^2R$. Te veel warmte beschadigt de isolatie, verkort de levensduur van
componenten en kan brand veroorzaken. Daarom moet je kabels correct dimensioneren, voldoende
koeling voorzien en overbelasting vermijden.

\textbf{Elektrodynamische gevaren.} Een stroom door een geleider wekt een magnetisch veld op. Lopen
er twee geleiders dicht bij elkaar, dan oefenen die velden \keyterm{mechanische krachten} op elkaar
uit. Bij grote stromen (denk aan een kortsluitstroom) worden die krachten zo groot dat de geleiders
bewegen of zelfs uit hun bevestiging losrukken. Dat leidt dan weer tot kortsluiting, schade aan
apparatuur of letsel door rondvliegende geleiders.

\begin{examenbox}
    Waarom is een \textbf{stevige mechanische bevestiging} van railkokers en dikke kabels zo
    belangrijk? Precies om die elektrodynamische krachten op te vangen. Het is dus geen
    afwerkingsdetail maar een veiligheidsmaatregel.
\end{examenbox}

\subsubsection{Directe gevaren voor mensen}

Wanneer er stroom door het lichaam loopt, spreken we van een \keyterm{elektrische schok}. De
gevolgen lopen van een lichte tinteling tot de dood:

\begin{itemize}
    \item \textbf{Zenuwstelsel:} de stroom verstoort de elektrische signalen in het lichaam.
          Dat geeft spierverkramping, ademhalingsproblemen of \belangrijk{ventrikelfibrillatie}
          (het hart trilt maar pompt niet meer).
    \item \textbf{Brandwonden:} bij hoge stromen of langdurige blootstelling verhit het weefsel.
          Dat kan zowel uitwendig als \emph{inwendig} zijn, en inwendige brandwonden zijn veel
          gevaarlijker omdat je ze niet ziet.
    \item \textbf{Ionisatie:} de stroom verandert de chemische samenstelling van cellen.
\end{itemize}

Hoe ernstig het wordt, hangt af van \textbf{zes factoren}:

\begin{enumerate}
    \item \keyterm{Stroomsterkte $I$} --- hoe hoger, hoe meer schade.
    \item \keyterm{Lichaamsimpedantie $Z_B$} --- bepaalt hoeveel stroom er bij een gegeven spanning
          door je lichaam gaat. Droge huid heeft een veel hogere weerstand dan natte huid;
          oneffenheden in de huid vullen zich met vocht en vormen dan een pad van lage weerstand.
    \item \keyterm{Frequentie $f$} --- $50$ tot $60$~Hz is uitgerekend het gevaarlijkst, omdat dat
          dicht bij het natuurlijke ritme van hart en zenuwstelsel ligt. Bij veel hogere frequenties
          kunnen de spieren simpelweg niet meer volgen en is het effect kleiner.
    \item \keyterm{Duur $t$} --- hoe langer de blootstelling, hoe groter de schade.
    \item \keyterm{Spanningsniveau $V$} --- een hogere spanning drijft bij dezelfde $Z_B$ een grotere
          stroom door het lichaam.
    \item \keyterm{Pad door het lichaam} --- loopt de stroom van hand naar hand of van hand naar
          voet, dan gaat hij door het hart. Dat is veel gevaarlijker dan van voet naar voet.
\end{enumerate}

\begin{waarschuwing}[Niet de spanning maar de stroom doodt]
    Het is de \belangrijk{stroom door het lichaam} die de schade doet, en die volgt uit
    $I_B = V/Z_B$. Daarom zijn de normen opgesteld in stroom (mA), en pas daarna omgerekend naar
    een aanraakspanning via een conventionele lichaamsimpedantie.
\end{waarschuwing}

\subsubsection{De AC-zonegrafiek (IEC 60479)}

Deze grafiek zet de \keyterm{stroom door het lichaam} uit tegen de \keyterm{blootstellingsduur},
allebei logaritmisch. De combinatie van beide bepaalt de ernst, niet de stroom alleen.
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=1.0]
        ...
        % x-ticks (logaritmisch, schematisch)
    ...
    \end{tikzpicture}
    \caption{Effect van wisselstroom op het menselijk lichaam volgens IEC 60479, schematisch (assen
        logaritmisch, niet op schaal). Beide assen samen bepalen de zone.}
    \label{fig:ac-zones}
\end{figure}
\FloatBarrier

\begin{itemize}
    \item \keyterm{AC-1} --- geen waarneembaar effect. Onder de waarnemingsdrempel (curve a,
          ongeveer $0{,}5$~mA).
    \item \keyterm{AC-2} --- waarneembaar, lichte spiersamentrekking, maar geen gevaarlijk
          fysiologisch effect. Begrensd door curve b, de drempel van de spierreactie.
    \item \keyterm{AC-3} --- spierverkramping: de persoon \belangrijk{kan de geleider niet meer
              loslaten} en krijgt ademhalingsmoeilijkheden. De effecten zijn nog \emph{omkeerbaar}.
    \item \keyterm{AC-4} --- kans op \belangrijk{ventrikelfibrillatie}, dus onomkeerbaar:
          \begin{itemize}
              \item AC-4.1: tot $5\%$ kans (curve $c_2$)
              \item AC-4.2: tot $50\%$ kans (curve $c_3$)
              \item AC-4.3: meer dan $50\%$ kans
          \end{itemize}
\end{itemize}

De curves zelf: \textbf{a} $=$ waarnemingsdrempel, \textbf{b} $=$ drempel van de spierreactie,
\textbf{$c_1$} $=$ fibrillatie onwaarschijnlijk, \textbf{$c_2$} $=$ $5\%$-drempel,
\textbf{$c_3$} $=$ $50\%$-drempel.

\begin{examenbox}[EXAMENVRAAG:]
    \textit{Benoem de assen van deze grafiek en leg uit waarom ze er zo uitziet.}

    De grenzen lopen schuin omdat hoe langer de stroom loopt, hoe kleiner hij mag zijn voor
    hetzelfde effect. Bij zeer korte tijden ($<10$~ms) is er nauwelijks effect, omdat de stroom dan
    buiten de kwetsbare fase van de hartcyclus (de T-golf) valt. \keyterm{Dat is precies waarom een
        aardlekschakelaar met een uitschakeltijd onder $30$~ms zo doeltreffend is.}
\end{examenbox}

\subsubsection{Van stroom naar aanraakspanning}

In de praktijk kan je de stroom door een lichaam niet meten of voorspellen; wat je wel kent, is de
\keyterm{aanraakspanning}. Daarom rekent men de stroomcurves om via een conventionele
lichaamsimpedantie:
\[I_B = \frac{V_{\text{aanraak}}}{Z_B}\]
Met $I_B$ de stroom door het lichaam in A, $V_{\text{aanraak}}$ de aanraakspanning in V en $Z_B$ de
lichaamsimpedantie in $\Omega$.

Elke lichaamsconditie heeft zijn eigen $Z_B$ en dus zijn eigen kromme:

\begin{center}
    \begin{tabular}{@{}llr@{}}
        \toprule
        \textbf{Code} & \textbf{Conditie}                          & \textbf{Grens (AC)} \\
        \midrule
        BB1           & Droge huid, beperkt contact                & $50$~V              \\
        BB2           & Natte huid                                 & $25$~V              \\
        BB3           & Volledig ondergedompeld (bad, zwembad)     & $12$~V              \\
        \bottomrule
    \end{tabular}
\end{center}

In gelijkstroom liggen die grenzen hoger: $120$~V, $60$~V en $30$~V.

\begin{examenbox}[EXAMENVRAAG:]
    \textit{Wat is het verband tussen de stroom-tijdgrafiek en de spanning-tijdgrafiek?}

    Het zijn \belangrijk{twee weergaven van hetzelfde fenomeen}. De AC-zonegrafiek is de
    fysiologische grondwaarheid (wat doet een stroom met het hart), de spanningsgrafiek is daaruit
    afgeleid via $I_B = V/Z_B$. De vorm is in beide grafieken dezelfde --- hoe langer de
    blootstelling, hoe lager de toegelaten waarde --- omdat de onderliggende fysiologie identiek is.
    Onder de conventionele grenswaarde mag de spanning \emph{onbeperkt} aanwezig blijven; dat is
    het verticale stuk links.
\end{examenbox}

\subsubsection{Indirecte gevaren}

Deze worden vaak \belangrijk{onderschat}. Twee typische situaties:

\textbf{Val of bewusteloosheid.} Iemand die een schok krijgt, kan vallen of het bewustzijn
verliezen. Het letsel komt dan van de val, niet van de stroom. Gebruik altijd \emph{isolerend}
materiaal om het slachtoffer aan te raken of te verplaatsen, anders krijg je zelf ook een schok.

\textbf{Vlamboog.} Raken koperen geleiders oververhit door een zeer hoge stroom, dan ioniseert de
lucht ertussen. Dat geeft in één klap vier gevaren tegelijk:
\begin{itemize}
    \item stukken koper worden weggeslingerd (letsel);
    \item koper sublimeert tot \emph{giftige gassen} (vergiftiging);
    \item er ontstaat intens licht als bij een lasboog (oogschade);
    \item de drukgolf geeft een harde knal (gehoorschade, fysieke schade).
\end{itemize}

\subsection{Veilig werken met elektriciteit}

\subsubsection{De vijf procedurestappen}

\begin{examenbox}[EXAMENVRAAG:]
    \textit{Wat zijn de vijf gouden regels van elektrisch schakelen?} Let op: \keyterm{de volgorde
        is niet willekeurig}, elke stap dekt het falen van de vorige af.
\end{examenbox}

\begin{enumerate}
    \item \keyterm{Volledig scheiden} van alle spanningsbronnen. Alle polen, alle voedingen ---
          denk ook aan noodstroom, UPS en opgeladen condensatoren.
    \item \keyterm{Beveiligen tegen herinschakeling}: vergrendelen en labelen
          (lock-out / tag-out), zodat niemand anders de installatie weer aanzet.
    \item \keyterm{Spanningsloosheid vaststellen} met een gekeurde spanningsaanwijzer. Test dat
          toestel \emph{voor en na} op een gekende spanning, anders weet je niet of het zelf werkt.
    \item \keyterm{Aarden en kortsluiten} van de geleiders (verplicht bij hoogspanning, aanbevolen
          bij laagspanning).
    \item \keyterm{Afschermen of afbakenen} van nabijgelegen delen die wél onder spanning staan, en
          de werkzone signaleren.
\end{enumerate}

Daarnaast: draag de juiste \textbf{persoonlijke beschermingsmiddelen} (isolerende handschoenen,
helm, gezichtsscherm, vlambestendige kleding) en \belangrijk{werk nooit alleen}.

\subsubsection{Veilige installaties}

Veiligheid begint niet bij het werken maar bij het \keyterm{ontwerp}:
\begin{itemize}
    \item \textbf{Ontwerp:} bereken en selecteer de juiste componenten voor de situatie
          (kabeldoorsnede, schakelvermogen, \dots).
    \item \textbf{Beschermingsapparatuur:} zekeringen, automaten en aardlekschakelaars installeren.
    \item \textbf{Onderbreken bij gevaar:} componenten die de stroom automatisch afsluiten wanneer
          er iets misloopt.
\end{itemize}

\subsubsection{Regelgeving}

\begin{itemize}
    \item \keyterm{EU Low Voltage Directive} (laagspanningsrichtlijn) --- Europese richtlijn voor
          laagspanningsapparatuur.
    \item \keyterm{AREI} (Algemeen Reglement op de Elektrische Installaties) en de Codex ---
          gedetailleerde regels op Belgisch niveau.
    \item \keyterm{IEC} (internationaal) en \keyterm{BEC} (Belgisch) --- normen voor veiligheid en
          compatibiliteit.
    \item \keyterm{Keurmerken} zoals CE --- een systeem krijgt zo'n merk pas als het aan de
          bijhorende normen voldoet.
\end{itemize}

Verder wordt een installatie aangepast aan de \emph{omgeving} waarin ze staat. Daarvoor bestaan
omgevingscodes: BB voor de lichaamsconditie (zie hierboven), BA voor de aanwezige personen ---
bijvoorbeeld een kinderopvang, waar strengere eisen gelden.
...
\begin{center}
    \begin{tabular}{@{}lll@{}}
        \toprule
        \textbf{Klasse}          & \textbf{AC}                        & \textbf{DC}          \\
        \midrule
        Hoogspanning             & $> 1000$~V$_{\text{eff}}$          & $> 1500$~V           \\
        Laagspanning             & $50$ -- $1000$~V$_{\text{eff}}$    & $120$ -- $1500$~V    \\
        Zeer lage spanning (ELV) & $< 50$~V$_{\text{eff}}$            & $< 120$~V            \\
        \bottomrule
    \end{tabular}
\end{center}

\keyterm{De grens van $50$~V AC is geen toeval:} dat is exact de conventionele aanraakspanningsgrens
voor BB1.

\subsection{Bescherming tegen overstroom}

\subsubsection{Overstroom versus kortsluiting}

\begin{examenbox}[EXAMENVRAAG:]
    \textit{Wat is het verschil tussen overstroom en kortsluiting? Welke toestellen beschermen
        ertegen? Schakelen die op dezelfde manier uit?} Deze vraag komt op bijna elk examen terug.
\end{examenbox}

\begin{center}
    \begin{tabular}{@{}p{22mm}p{50mm}p{50mm}@{}}
        \toprule
                              & \textbf{Overstroom (overbelasting)}                                                                                          & \textbf{Kortsluitstroom}                                                                                       \\
        \midrule
        \textbf{Oorzaak}      & Te veel last op één kring, motor te zwaar belast. De stroom volgt nog de \emph{normale} weg                                   & Directe verbinding tussen fase en nul of aarde door defecte isolatie. De stroom neemt een \emph{kortere} weg    \\[1mm]
        \textbf{Grootte}      & Iets tot enkele malen $I_n$                                                                                                  & Tot honderden malen $I_n$                                                                                      \\[1mm]
        \textbf{Gevolg}       & Geleidelijke oververhitting, isolatie degradeert, brandgevaar op termijn                                                      & Onmiddellijke vlamboog, explosie, brand                                                                        \\[1mm]
        \textbf{Beveiliging}  & \keyterm{Thermisch:} bimetaal in de MCB of een thermisch relais                                                               & \keyterm{Magnetisch:} elektromagnetische spoel in de MCB, of een smeltveiligheid                                \\[1mm]
        \textbf{Uitschakelen} & \belangrijk{Vertraagd}, invers-tijdkarakteristiek: hoe groter de overstroom, hoe sneller                                      & \belangrijk{Vrijwel onmiddellijk} ($<10$~ms), quasi onafhankelijk van de stroomgrootte                          \\
        \bottomrule
    \end{tabular}
\end{center}

\textbf{Waarom die uitschakeltijden verschillen} --- dat is de kern van de vraag:

\textbf{Bij overbelasting mag je niet direct uitschakelen.} Een motor trekt bij het starten
kortstondig $5$ tot $8$ keer zijn nominale stroom. Dat is ongevaarlijk zolang het kort duurt. Het
bimetaal warmt op met een tijdconstante van seconden tot minuten, dus tijdelijke pieken worden
genegeerd en een aanhoudende overbelasting wordt wél onderbroken.

\textbf{Bij kortsluiting moet je zo snel mogelijk uitschakelen.} De energie die in de fout
gedissipeerd wordt is $\int i^2 R\,dt$. Met een enorme $i$ loopt dat in milliseconden op tot schade.
De elektromagnetische spoel reageert daarom quasi ogenblikkelijk.

\subsubsection{De MCB-karakteristiek}

Een \keyterm{MCB} (Miniature Circuit Breaker, installatieautomaat) combineert beide mechanismen in
één toestel. Hij is automatisch en herbruikbaar, in tegenstelling tot een smeltveiligheid.

\begin{figure}[ht]
    \centering
    \begin{tikzpicture}[scale=1.0]
        \draw[->, thick] (0,0) -- (9.5,0) node[right] {$I / I_n$};
        \draw[->, thick] (0,0) -- (0,5.6) node[above] {$t$ [s]};
        \foreach \x/\l in {1.2/1, 3.0/2, 4.6/3, 6.2/5, 7.6/10}
        \draw (\x,0) -- (\x,-0.12) node[below, font=\scriptsize] {\l};
        \foreach \y/\l in {0.6/0.01, 1.8/1, 3.0/60, 4.4/3600}
        \draw (0,\y) -- (-0.12,\y) node[left, font=\scriptsize] {\l};
        % thermisch deel: dalende invers-tijdkromme
        \draw[very thick, schoolOrange]
        (1.5,5.2) .. controls (2.4,4.0) and (3.4,2.4) .. (5.4,1.3);
        % overgang
        \draw[very thick, schoolOrange] (5.4,1.3) -- (6.2,1.15);
        % magnetisch deel: quasi verticaal
        \draw[very thick, schoolRed] (6.2,1.15) -- (6.2,0.55) -- (8.8,0.5);
        % annotaties
        \node[font=\small\bfseries, schoolOrange, align=center] at (2.9,4.6)
        {thermisch\\(bimetaal)};
        \node[font=\small\bfseries, schoolRed, align=center] at (7.9,2.2)
        {magnetisch\\(spoel)};
        \draw[<->, schoolGray] (6.2,-0.75) -- (7.6,-0.75)
        node[midway, below, font=\scriptsize] {type B/C/D};
    \end{tikzpicture}
    \caption{Uitschakelkarakteristiek van een MCB, schematisch (assen logaritmisch). Links het
        thermische deel met invers-tijdkromme, rechts het magnetische deel met een quasi verticale lijn.}
    \label{fig:mcb-karakteristiek}
\end{figure}
\FloatBarrier

\begin{examenbox}[EXAMENVRAAG:]
    \textit{Benoem de assen van de MCB-karakteristiek en verklaar de vorm.}

    \textbf{Assen:} horizontaal de \keyterm{stroom} (als veelvoud van $I_n$, logaritmisch),
    verticaal de \keyterm{uitschakeltijd} in seconden (logaritmisch).

    \textbf{Linkerdeel (thermisch, dalend):} bij overbelasting warmt het bimetaal op volgens
    $\int i^2R\,dt$. Hoe groter de overstroom, hoe sneller de drempel bereikt is --- vandaar de
    \keyterm{invers-tijdkarakteristiek}.

    \textbf{Rechterdeel (magnetisch, quasi verticaal):} boven een drempel trekt de
    elektromagnetische spoel de contacten ogenblikkelijk open. De tijd is dan $<10$~ms en zo goed
    als onafhankelijk van de stroomgrootte.

    \textbf{De overgang} tussen beide gebieden wordt bepaald door het \keyterm{type}:
    type B schakelt magnetisch bij $3$ tot $5\,I_n$, type C bij $5$ tot $10\,I_n$, type D bij $10$
    tot $20\,I_n$. Dat verklaart waarom er meerdere krommen op de grafiek staan.
\end{examenbox}

\textbf{Smeltveiligheid (fuse).} Een smeltdraad die doorbrandt bij te hoge stroom. Zeer betrouwbaar
en snel bij kortsluiting, maar \belangrijk{niet herbruikbaar}.

\subsection{Bescherming tegen elektrische schokken}

\subsubsection{Preventie}

Voorkomen is beter dan genezen. Direct of indirect contact vermijd je door:
\begin{itemize}
    \item \keyterm{Isolatie} --- geleiders omhullen zodat direct contact onmogelijk is.
    \item \keyterm{Behuizing en obstakels} --- gevaarlijke componenten fysiek afschermen en de
          toegang bemoeilijken.
\end{itemize}

\subsubsection{Aarding en equipotentiaalverbindingen}

\textbf{Aarden} is de installatie met de aarde verbinden, zodat foutstromen veilig kunnen wegvloeien
en de spanning op metalen delen beperkt blijft.

\textbf{Equipotentiaalverbindingen} verbinden geleidende delen onderling, zodat ze allemaal op
\emph{dezelfde} potentiaal staan. Is er geen spanningsverschil, dan kan er ook geen stroom door je
lichaam lopen wanneer je twee delen tegelijk aanraakt.

\subsubsection{Veilige zeer lage spanning}

Voed een systeem met een spanning onder $50$~V AC en er kan bij aanraking simpelweg geen
gevaarlijke stroom vloeien. Dit heet \keyterm{SELV} (Safety Extra-Low Voltage) en is de reden
waarom bijvoorbeeld badkamerverlichting op $12$~V werkt.

\subsubsection{Aardlekschakelaar (RCD)}

\begin{examenbox}[EXAMENVRAAG:]
    \textit{Wat is een RCD? Waartegen beschermt het? Geef synoniemen.}
\end{examenbox}

\keyterm{RCD} staat voor \emph{Residual Current Device}. In het Nederlands: \keyterm{aardlekschakelaar}
of \keyterm{verliesstroomschakelaar}. Andere benamingen: differentieelschakelaar, RCCB
(Residual Current Circuit Breaker).

\textbf{Werking.} Het toestel meet met een somstroomtransformator de \keyterm{som} van de stromen
door alle actieve geleiders (fase(n) én nulgeleider). Normaal is die som nul: wat er heen gaat, komt
ook terug. Lekt er stroom weg naar de aarde --- door een isolatiefout of door een mens die de fase
aanraakt --- dan klopt de balans niet meer:
\[\underline{I}_{\text{fase}} + \underline{I}_{\text{nul}} = \underline{I}_{\Delta} \neq 0\]

Overschrijdt $I_\Delta$ de gevoeligheid van het toestel, dan schakelt het binnen $<30$~ms uit.
Gangbare waarden: \keyterm{$30$~mA} voor persoonsbeveiliging, $300$ of $500$~mA voor brandbeveiliging.

\begin{waarschuwing}[Waar een RCD níet tegen beschermt]
    \begin{itemize}
        \item Niet tegen \belangrijk{overbelasting} of \belangrijk{kortsluiting} --- daar dienen de
              MCB en de smeltveiligheid voor. Beide zijn dus nodig.
        \item Niet als je fase én nulgeleider tegelijk vastneemt: de som blijft dan nul, dus het
              toestel ziet geen verschil.
    \end{itemize}
\end{waarschuwing}

\keyterm{Waarom $30$~mA en $<30$~ms?} Dat werkpunt ligt in zone AC-3 van de IEC-60479-grafiek
(figuur~\ref{fig:ac-zones}): boven de waarnemingsdrempel maar \emph{onder} de fibrillatiedrempel,
zodat de effecten omkeerbaar blijven. De grafiek is dus geen theorie maar de rechtstreekse
