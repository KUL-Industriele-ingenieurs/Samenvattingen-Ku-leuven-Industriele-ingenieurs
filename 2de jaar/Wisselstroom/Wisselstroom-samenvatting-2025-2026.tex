...
Dit vak is voor negen tiende complexe rekenkunde. Ken je je toestel goed? Dan werk je op het examen
bijna niets met de hand uit. Dat scheelt tijd \'en rekenfouten. Zet je toestel dus \'e\'en keer goed
op v\'o\'or je begint te oefenen.

\begin{theorie}[De vier instellingen die je moet controleren]
    \begin{enumerate}
        \item \textbf{Complexe modus} aan (op de meeste toestellen \texttt{CMPLX} of \texttt{Complex}).
              Zonder die modus geeft je toestel een foutmelding zodra er een $j$ in staat.
        \item \textbf{Graden of radialen}: kies \'e\'en van beide en blijf consequent. De cursus werkt in graden
              ($\angle 30^\circ$), dus zet \texttt{DEG}. De klassieke fout: je tikt een hoek in graden
              in terwijl je toestel in \texttt{RAD} staat.
        \item \textbf{Weergavevorm}: kies of je het antwoord in $a+jb$ of in $r\angle\theta$ ziet.
              Meestal schakel je per berekening om. Doe dat ook, want je hebt beide vormen nodig.
        \item \textbf{Vast aantal cijfers} (bv.\ \texttt{FIX 3} of \texttt{Norm 2}). Anders sleep je
              afrondingsfouten mee doorheen een lange oefening.
    \end{enumerate}
\end{theorie}
...
\begin{theorie}[Wanneer is een stelsel oplosbaar?]
    Een stelsel is pas oplosbaar als je \textbf{evenveel onafhankelijke vergelijkingen hebt als
    onbekenden}. Tel dus eerst, v\'o\'or je begint te rekenen:
    \[\#\text{vergelijkingen} = \#\text{onbekenden}\]
    \begin{itemize}
        \item \textbf{Te weinig} vergelijkingen $\rightarrow$ oneindig veel oplossingen. Je bent een
              knoop of een lus vergeten, of je hebt een gegeven niet gebruikt.
        \item \textbf{Te veel} vergelijkingen $\rightarrow$ minstens \'e\'en ervan herhaalt de andere.
              Bij knooppuntanalyse is de KCL in de \emph{referentieknoop} altijd overbodig. Die
              vergelijking volgt automatisch uit de andere. Laat ze dus weg.
    \end{itemize}
\end{theorie}

\belangrijk{``Onafhankelijk'' is het sleutelwoord.} Twee vergelijkingen die veelvouden van elkaar
zijn, tellen samen maar voor \'e\'en. Je toestel merkt dat zelf. Het geeft een foutmelding
(\texttt{Math ERROR} of \texttt{Infinite Solutions}) omdat de determinant nul is. Krijg je die
melding? Zoek dan niet naar een tikfout, maar naar een \emph{dubbele} vergelijking.

\textbf{Hoeveel onbekenden heb je?} Dat volgt rechtstreeks uit je methode:
\begin{itemize}
...
\begin{waarschuwing}[Twee valkuilen bij hoeken]
    \begin{itemize}
        \item Je toestel geeft $\arg$ altijd in het interval $(-180^\circ, 180^\circ]$. Fysisch kan de
              werkelijke hoek daarbuiten liggen. Bepaal het kwadrant dus zelf via de tekens van $a$
              en $b$.
        \item $\arctan(X/R)$ met de hand geeft hetzelfde probleem. $\arctan$ ziet het verschil tussen
              het tweede en het vierde kwadrant niet. Werk daarom rechtstreeks met
              $\underline{Z} = R + jX$ in complexe modus.
    \end{itemize}
\end{waarschuwing}
...
Figuur~\ref{fig:golfvormen-kf} toont voor vier golfvormen waar $A_m$, $A_{\text{eff}}$ en
$A_{\text{gem}}$ liggen. Zo zie je meteen waarom de getallen in de tabel eronder verschillen.
...
Dat verschil merk je in de praktijk. De goedkope meter meet $A_{\text{gem}}$ en toont
$A_{\text{gem}} \times 1{,}11$. Hij \emph{gokt} dus dat je golf een zuivere sinus is. Hangt er een
dimmer, een schakelende voeding of een frequentieregelaar aan? Dan is de golf vervormd
(figuur~\ref{fig:vervormd-signaal}) en is de echte vormfactor niet meer $1{,}11$. De aflezing klopt
dan niet. Op een toestel dat het wél goed doet staat daarom \textbf{TRUE RMS} gedrukt.
...
\begin{examenbox}[Reken de vormfactor zelf uit]
    ...
    draaispoelmeter (leest $A_{\text{gem}}$) en met een TRUE RMS-meter (leest $A_{\text{eff}}$). De
    vormfactor volgt uit de verhouding van beide aflezingen. Krijg je iets anders dan $1{,}11$? Dan
    weet je zeker dat het signaal geen zuivere sinus is.
\end{examenbox}
...
\begin{conceptbox}[Impedantie en reactantie]
    ...
    Verwar reactantie niet met \emph{reluctantie}. Reluctantie is de magnetische weerstand van een
    ijzeren kern en hoort niet in dit hoofdstuk thuis.
\end{conceptbox}
...
\begin{waarschuwing}[Let op het verschil tussen een fasor en een grootte]
    ...
    \begin{itemize}
        \item \textbf{Een onderstreepte letter} ($\underline{V}$, $\underline{I}$, $\underline{S}$,
              $\underline{Z}$) is een \textbf{complex} getal met een grootte \emph{en} een hoek.
              Daarmee mag je optellen, delen en vermenigvuldigen zoals je gewend bent.
        ...
        \item \textbf{$P$, $Q$, $R$, $X$ en $\cos\phi$} zijn gewone re\"ele getallen. Die mag je wel
              optellen, want ze zijn al een projectie op \'e\'en as.
    \end{itemize}
    Vuistregel: zodra er een hoek in het spel zit, hoort er een streep onder de letter.
\end{waarschuwing}

De stroom en de spanning oscilleren in de tijd. Het vermogen oscilleert dus ook.
...
Afhankelijk van het circuit ijlt de stroom voor of na op de spanning.
...
\begin{warningbox}[Schijnbaar vermogen of complex vermogen?]
    Twee namen die op elkaar lijken, maar niet hetzelfde betekenen:
    \begin{itemize}
        \item $\underline{S} = \underline{V}\,\underline{I}^{*} = P + jQ$ is het
              \textbf{complex vermogen}: een fasor, met een grootte \'en een hoek.
        \item $|\underline{S}| = V_{\text{eff}} I_{\text{eff}}$ is het \textbf{schijnbaar
                  vermogen}: enkel de grootte, de schuine zijde van de vermogendriehoek.
    \end{itemize}
    Beide staan in VA. Vandaar de verwarring. Het verschil telt, want alleen $\underline{S}$
    is optelbaar: $|\underline{S}_T| \neq |\underline{S}_1| + |\underline{S}_2|$, maar
    $\underline{S}_T = \underline{S}_1 + \underline{S}_2$ klopt wel.
\end{warningbox}
...
Daarmee is de cirkel rond. $P = V_{\text{eff}} I_{\text{eff}}\cos\phi$ en
$Q = V_{\text{eff}} I_{\text{eff}}\sin\phi$ zijn geen aparte definities om vanbuiten te leren. Het
zijn het re\"ele en het imaginaire deel van hetzelfde complexe getal $\underline{S}$. En
$|\underline{S}| = V_{\text{eff}} I_{\text{eff}}$ is de \emph{lengte} ervan. Die lengte heeft geen
hoek en is dus niet zomaar optelbaar met een ander schijnbaar vermogen.
...
\begin{examenbox}[Eindcontrole die je altijd moet doen]
    ...
    Klopt dat niet? Dan zit er ergens een rekenfout. Deze controle kost tien seconden en redt je oefening.
\end{examenbox}
...
\begin{examenbox}
    ...
    straal $S$, niet langs een verticale lijn. Net dat onderscheid wordt gevraagd.
\end{examenbox}
...
Resonantie hangt dus af van de frequentie. Bij de ene frequentie krijg je resonantie, bij de andere
niet.

\begin{waarschuwing}[De wortelformule geldt niet altijd]
    Deze formule geldt alleen voor een \textbf{zuivere} serie- of parallelschakeling van $L$ en
    $C$. Zit er een weerstand \emph{binnen} de parallelschakeling? Dan verandert de effectieve
    reactantie en is de formule onbruikbaar.
...
\end{itemize}
\belangrijk{Kijk dus eerst naar de topologie} voor je zegt of een zone inductief of capacitief is.
...
Dit is de valkuil die op het examen terugkomt. Serie- en parallelresonantie treden allebei op als
$\text{Im}\{\underline{Z}\} = 0$. Het gevolg is wel tegengesteld:
...
Het enige verschil: je rekent nu met \textbf{complexe} impedanties $\underline{Z}$ in plaats van met
reële weerstanden $R$. De regels blijven dezelfde. Alleen je rekenmachine moet in complexe modus
staan.

Nieuw zijn wel de \emph{werkwijzen} verderop in dit hoofdstuk: wanneer je achterwaarts rekent met
een correctiefactor, wanneer je knooppuntanalyse gebruikt, en hoe je een oefening aanpakt waarin je
alleen de \emph{groottes} van stromen kent.
...
Laat je niet in de war brengen door de imaginaire impedanties van condensatoren en spoelen. Je
gebruikt dezelfde regels als bij DC-netwerken.
...
\begin{examenbox}
    Bekijk de slides van dit hoofdstuk goed.
    Thevenin en Norton komen hier opnieuw aan bod.
    Begreep je vorig jaar Elektrische Netwerken? Dan lukt dit ook.
\end{examenbox}

\subsection{Spanningsdeling en stroomdeling}

Deze twee gebruik je in bijna elke 1-fase oefening. Ze moeten er dus vlot in zitten.
...
\begin{waarschuwing}[Let op welke impedantie in de teller staat]
    ...
    Dat is logisch. Door de tak met de \emph{kleinste} impedantie loopt de \emph{grootste} stroom. De
    breuk moet dus klein worden als je eigen impedantie groot is.
\end{waarschuwing}
...
\begin{oefening}[Sadiku 10.16: twee stroombronnen wegwerken met brontransformatie]
    ...
    \textbf{Kerninzicht.} Brontransformatie is geen rekentruc, maar een \emph{topologietruc}. Elke
    omzetting haalt \'e\'en knooppunt uit het net weg. Blijft er maar \'e\'en lus over? Dan heb je
...
\begin{oefening}[Voorbeeld uit slides: achterwaarts rekenen met een correctiefactor]
    ...
    \end{enumerate}
    De lijn ``kost'' dus ongeveer $17$~V, ruim $7\%$ spanningsval. In de praktijk is dat te veel: de
...
\begin{examenbox}[EXAMENVRAAG:]
    ...
    Is $\underline{k}$ reëel? Dan veranderen de hoeken niet en wordt het hele fasordiagramma alleen
    groter of kleiner. Is $\underline{k}$ complex? Dan draait het hele diagramma over
    $\arg(\underline{k})$, maar de \textbf{onderlinge} hoeken blijven gelijk. Net die onderlinge
    hoeken bepalen de arbeidsfactoren en de vermogens. Corrigeer dus nooit hoek per hoek.
\end{examenbox}
...
\begin{waarschuwing}[Knooppunt- of lusanalyse: welke kies je?]
    \begin{itemize}
        ...
        \item Een stroombron in een \emph{buitentak} geeft je \'e\'en kringstroom gratis. Een
              spanningsbron rechtstreeks aan de referentieknoop geeft je \'e\'en knoopspanning
              gratis. Kijk dus eerst welke bronnen je een vergelijking besparen.
    \end{itemize}
\end{waarschuwing}
...
\begin{oefening}[Sadiku 11.85: driedradennet met drie lussen --- werk met SysSolve]
    ...
    met complexe co\"effici\"enten. Met de hand elimineren duurt lang en gaat gegarandeerd mis.
...
Je moet dit bewijs niet kunnen reproduceren. Volg de afleiding wel \'e\'en keer. Dan hoef je
$\underline{Z}_L = \underline{Z}_{\text{th}}^{*}$ nooit meer vanbuiten te leren, want je ziet
...
\begin{waarschuwing}[Dit is geen rendement van 100\,\%]
    Bij aanpassing staat er evenveel weerstand in de bron als in de last. De bron verstookt dus
    ...
    omgekeerde ($R_{\text{th}} \ll R_L$). Aanpassing gebruik je bij signalen en antennes. Daar telt
    het vermogen zelf, niet de energiefactuur.
\end{waarschuwing}
...
Stel je drie bronnen voor die onderling telkens 120 graden verschillen.

\begin{examenbox}[Wat ``de voeding'' in het labo betekent]
    Spreekt een opgave over de labovoeding? Dan gaat het over een driefasig net met een
...
\begin{conceptbox}[Waarom gebruiken we 3-fasige systemen?]
    De industrie en de energiedistributie werken overal met driefasige wisselstroom. Daar zijn drie
    doorslaggevende redenen voor:
    \begin{enumerate}
        ...
        \item \textbf{Drastische koper- en kabelbesparing:} wil je hetzelfde totale vermogen transporteren over
              dezelfde afstand, bij dezelfde lijnspanning en hetzelfde toelaatbare leidingverlies? Dan heeft een
              3-fase net tot \textbf{$25\%$ à $50\%$ minder geleidermateriaal} nodig dan een gelijkwaardig 1-fase net.
        \item \textbf{Ronddraaiend magnetisch draaiveld:} drie symmetrische fasewikkelingen liggen fysiek $120^\circ$
              verschoven in de stator. Ze wekken vanzelf een continu roterend magnetisch draaiveld op. Daardoor
              bestaan er robuuste, betrouwbare en zelfstartende asynchrone kooiankermotoren.
    \end{enumerate}
\end{conceptbox}
...
Zijn de belastingen gelijk? Dan loopt er \textbf{geen circulatiestroom} rond in de driehoek zelf.
De drie bronspanningen heffen elkaar op ($\vec{V}_1 + \vec{V}_2 + \vec{V}_3 = 0$). Er is dus geen
netto spanning die zo'n stroom kan drijven.
...
\begin{waarschuwing}[De 30 graden zijn geen detail]
    ...
    Vergeet je die? Dan zit je $30^\circ$ fout op je fasehoek, en dus ook op je arbeidsfactor en je
    vermogens.
    ...
    dezelfde aftrekking. Daarom kan je ze niet los van elkaar onthouden.
...
\subsubsection{Denkmethode: maak van elk 3-fase net één enkele lus}

Bijna elke 3-fase oefening los je op met hetzelfde recept. Herleid het net tot het
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[stap, below=5mm of e] (f) {\textbf{6. De andere twee gratis.} $\underline{I}_b = \underline{I}_a\angle -120^\circ$, $\underline{I}_c = \underline{I}_a\angle +120^\circ$.};
        \foreach \x/\y in {a/b, b/c, c/d, d/e, e/f} \draw[pijl] (\x) -- (\y);
    \end{tikzpicture}
    \caption{Het recept voor vrijwel elke 3-fase oefening. Stap 1 tot en met 3 is puur omzetwerk.
        Vanaf stap 4 is het een eenvoudige serieschakeling.}
    \label{fig:recept-3fase}
\end{figure}
\FloatBarrier

\begin{waarschuwing}[Wat je op het einde nog moet terugrekenen]
    Het eenfase-equivalent geeft je de \textbf{lijnstromen}. Vraagt de opgave naar iets \emph{binnen}
    de oorspronkelijke driehoek? Dan moet je terugrekenen:
    \[I_{f,\Delta} = \frac{I_L}{\sqrt{3}} \quad\text{met } +30^\circ
        \qquad\qquad
        V_{f,\Delta} = V_L\]
    Het \textbf{vermogen} mag je wél rechtstreeks uit het equivalent halen: $P = \sqrt{3}V_LI_L\cos\phi$
    geldt voor ster én driehoek.
\end{waarschuwing}
...
\belangrijk{Dit is de ster-driehoekstarter.} Schakel dezelfde motorwikkelingen van ster naar
driehoek. Hun impedantie wordt dan 3$\times$ kleiner: 3$\times$ meer stroom en 3$\times$ meer
vermogen. Starten doe je in ster (zacht), draaien in driehoek (vol vermogen).
...
\subsubsection{Twee toepassingen van het recept}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/vboefening.png}
    \caption{Y-bron met transmissielijn en $\Delta$-belasting.}
...
\begin{oefenblok}[Y-bron, lijnimpedantie, $\Delta$-last]
    ...
    \stapreden{\textbf{Stap 6} --- de andere twee volgen gratis:}
...
\begin{figure}[ht]
    ...
    \caption{$\Delta$-bron met gemengde Y/$\Delta$-belasting.}
    \label{fig:vboef.png}
\end{figure}
\FloatBarrier
...
\subsubsection{Het idee in drie zinnen}

\begin{theorie}[Waarom twee wattmeters volstaan]
    Het totale vermogen is de som van de drie \textbf{fasevermogens}, elk gemeten t.o.v.\ het
    sterpunt $n$. In een driedradensysteem bestaat $n$ niet. Neem daarom \'e\'en van de lijnen als
    referentie, zeg $b$. De fasespanningen worden dan \textbf{lijnspanningen}. En omdat
    $\underline{I}_a + \underline{I}_b + \underline{I}_c = 0$, verdwijnt de derde meter vanzelf uit
    de som. Er blijft $P_{tot} = W_1 + W_2$ over: twee lijnspanningen maal twee lijnstromen.
\end{theorie}
...
driehoeklast is $I_f = I_L/\sqrt{3}$. Dat verschil is dus geen detail.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[line width=0.8pt, font=\small]
        ...
        \end{scope}
    \end{tikzpicture}
    \caption{De hele methode zit in deze ene stap: vervang het (vaak onbestaande) sterpunt $n$ als
        referentie door lijn $b$. De drie fasespanningen worden dan twee lijnspanningen, en de derde
...
\begin{waarschuwing}[Wanneer twee wattmeters níet volstaan]
    ...
    nulgeleider. Ze werkt ook bij onevenwicht. Dat is net haar sterkte.

    Ze werkt \belangrijk{niet} zodra er een \textbf{nulgeleider is waar stroom door loopt}
    (vierdraads en onevenwichtig). De afleiding steunt op $\underline{I}_a + \underline{I}_b +
        \underline{I}_c = 0$. Die som is dan niet meer nul. Je hebt dan \textbf{drie} wattmeters nodig.
\end{waarschuwing}
...
\begin{waarschuwing}[Alleen $W_1+W_2$ geldt altijd]
    $P_{tot} = W_1 + W_2$ volgt \textbf{enkel uit KCL} en geldt dus altijd: evenwichtig of niet,
    ster of driehoek.

    De formules voor $Q_{tot}$ en $\theta$ veronderstellen dat de drie fasen \textbf{identiek} zijn.
    Bij een ongebalanceerde last zijn ze ongeldig en krijg je zelfs verkeerde tekens.
    Bereken $Q$ dan per tak met $Q = I_f^2 X$ en tel op.
\end{waarschuwing}
...
\belangrijk{Dezelfde last in driehoek trekt dus drie keer zoveel vermogen als in ster.} Dat is het
principe achter de \textbf{ster-driehoekstarter} van een motor. Je laat hem aanlopen in ster (minder
stroom en koppel) en schakelt pas in regime over naar driehoek (vol vermogen).
...
\begin{examenbox}[EXAMENTIP: dit hoofdstuk zijn gratis punten]
    ...
    (somstroomtransformator), overbelasting versus kortsluiting of aanraakspanning. Je moet niets
    rekenen en niets afleiden: je kent het of je kent het niet. Dit is dus het goedkoopste stuk van
    de leerstof. Eén avond studeren levert een zekere score op.
...
\begin{figure}[H]
    ...
    \caption{Twee evenwijdige geleiders in doorsnede. Elke stroom maakt een magnetisch veld, en dat
        ...
        $i_1 i_2$, en dus met het \textbf{kwadraat} van de stroom. Bij normale bedrijfsstroom is ze
        daarom verwaarloosbaar. Bij een kortsluitpiek van tientallen kiloampères rukt ze railkokers
        uit hun steunen.}
    \label{fig:elektrodynamische-kracht}
\end{figure}
\FloatBarrier

Bij een zware kortsluitstroom (tot tientallen kiloampères) is deze kracht \textbf{evenredig met het
kwadraat van de piekstroom ($i_{\text{piek}}^2$)}. Geleiders en railkokers worden dan met enorme
kracht naar elkaar toe getrokken of uit elkaar geslagen. Dat scheurt kasten open of verbrijzelt
isolatiesteunen.
...
\begin{examenbox}[EXAMENVRAAG:]
    ...
    schalen met $I_{\text{kortsluit}}^2$. Zonder stevige steunen rukken de geleiders los. Dat geeft
    secundaire kortsluitingen en zware schade.
\end{examenbox}
...
\begin{wrapfigure}{r}{0.42\textwidth}
    \vspace{-4mm}
    \centering
    \includegraphics[width=0.4\textwidth]{assets/stroom-dodelijkheid.png}
...
\begin{waarschuwing}[Niet de spanning maar de stroom doodt!]
    ...
    tijdsduur $t$. Omdat $I_B = V_{\text{aanraak}}/Z_B$, drukken internationale veiligheidsnormen
    de grenzen eerst uit in milliampères (mA). Pas daarna rekenen ze die om naar spanningen (V).
\end{waarschuwing}
...
\begin{examenbox}
    Deze figuur moet je kunnen tekenen en uitleggen op het examen.
\end{examenbox}
...
\begin{examenbox}
    Deze figuur moet je ook kunnen tekenen en uitleggen op het examen.
    Niet verwarren met de IEC 60479-zonegrafiek: dit is een \textbf{aanraakspanningsgrafiek} die de
    veilige spanningsgrenzen aangeeft voor verschillende omgevingscondities.
\end{examenbox}
...
\textbf{Secundaire letsels (vallen/schrikreactie).} De spierreflex na een lichte schok kan je van een
ladder doen vallen of tegen draaiende machines duwen. Raak bij een redding het slachtoffer nooit met
blote handen aan zolang de spanning er nog op staat. Gebruik droog isolerend materiaal.
...
\begin{figure}[htbp]
    ...
    \caption{Dezelfde vijf stappen zoals ze in de cursus staan (``de Veilige Vijf''). Leer deze
        vijf woorden uit het hoofd. De examenvraag luidt letterlijk ``geef de vijf gouden
        regels''.}
...
\textbf{Uitwendige invloeden (AREI-codering):} elektrische apparatuur moet bestand zijn tegen haar
omgeving. Een 2-lettercode geeft dat aan:
\begin{itemize}
...
\begin{wrapfigure}{r}{0.46\textwidth}
    \vspace{-4mm}
    \centering
    \includegraphics[width=0.44\textwidth]{assets/overcurrents_in_iz_ib.png}
...
In de figuur betekenen de drie stromen:
\begin{itemize}
    \item $I_b$: de \textbf{ontwerpstroom van de belasting}; dit is de stroom die de
          aangesloten verbruikers normaal vragen.
    \item $I_n$: de \textbf{nominale stroom van de beveiliging}, bijvoorbeeld de
          nominale stroom van een MCB. De automaat moet bij normaal bedrijf niet
          uitschakelen.
    \item $I_z$: de \textbf{toelaatbare continue stroom van de kabel}; dit is de
          grootste stroom die de geleider langdurig mag voeren zonder gevaarlijke
          opwarming.
\end{itemize}
Daarom kies je de beveiliging zo dat
\[
    I_b \le I_n \le I_z
\]
met: $I_b$ = ontwerpstroom (A), $I_n$ = nominale stroom van de beveiliging (A) en
$I_z$ = toelaatbare continue kabelstroom (A). De belasting mag de automaat dus niet
onnodig doen uitschakelen, maar de automaat moet de kabel wel tegen overbelasting
beschermen.
...
De oudste vorm van overstroombeveiliging is de \textbf{smeltzekering} (fuse): een dun
geleiderdraadje in een porseleinen of glazen patroon. Bij een te grote stroom smelt het
draadje door en onderbreekt het de kring. Nadat de smeltzekering gesprongen is, moet je
ze vervangen. Een smeltzekering kan je dus niet opnieuw inschakelen.

\begin{wrapfigure}{r}{0.42\textwidth}
    \vspace{-4mm}
    \centering
    \includegraphics[width=0.39999999999999997\textwidth]{assets/fuses_smeltzekeringen.png}
...
\textbf{Samengevat:} een overbelasting ontstaat door langdurig te veel stroom in het
normale stroompad. Een kortsluiting ontstaat door een foutverbinding met zeer kleine
impedantie. Een MCB detecteert beide, maar met een ander mechanisme en een andere
uitschakelsnelheid. Een MCB vervangt geen RCD: de MCB beschermt de installatie tegen
overstroom, de RCD tegen lekstromen naar aarde.
...
minstens twee, met een verschillende gevoeligheid. Ze doen namelijk twee verschillende dingen:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/rcd_werking_extra.png}
    \caption{RCD bij een aardlek: de heen- en terugstroom zijn niet meer gelijk, waardoor
        de RCD de kring uitschakelt.}
    \label{fig:rcd-werking-extra}
\end{figure}
\FloatBarrier
...
\begin{waarschuwing}[Essentiële beperkingen van een RCD (Waar een RCD níet tegen beschermt)]
    \begin{enumerate}
        \item Een RCD biedt \belangrijk{geen bescherming tegen overbelasting of kortsluiting} tussen fase en nul!
              Loopt er $100\text{ A}$ tussen fase en nul zonder lekstroom naar aarde? Dan is $\sum I = 0$
              en tript de RCD nooit. Combineer een RCD daarom altijd met een MCB.
...
\begin{oefenblok}[Ongebalanceerde Delta-belasting analyse]
    ...
    Bereken de stromen in elk been van de Delta met de wet van Ohm ($\mathbf{I} = \mathbf{V}/\mathbf{Z}$).

    \textbf{Stroom AB:}
    $$\mathbf{I}_{AB} = \frac{\mathbf{V}_{ab}}{\mathbf{Z}_{AB}} = \frac{208\angle{0^\circ}}{20} = 10.4\angle{0^\circ} \text{ A}$$

    \textbf{Stroom BC:}
    Zet $\mathbf{Z}_{BC} = 10 - j10$ eerst om naar poolcoördinaten:
    ...
    \textbf{Stroom CA:}
    Zet $\mathbf{Z}_{CA} = 12 + j5$ eerst om naar poolcoördinaten:
...
\begin{oefenblok}[Onevenwichtige Delta-belasting met twee-wattmetermethode]
    ...
    Bereken eerst de fasestromen (de stromen door de weerstanden) met de wet van Ohm ($I = V/R$):
    ...
    Pas daarna de stroomwet van Kirchhoff toe in elk knooppunt:
    ...
    De belastingen zijn zuivere weerstanden, dus gebruik $P = V^2 / R$ en tel op:
    ...
    Voor het meten van het totale vermogen in een driedraadsysteem gebruik je twee wattmeters. Kies lijn B als referentielijn.
    ...
    Gebruik de formule $P = |\mathbf{V}| |\mathbf{I}| \cos(\phi)$, met $\phi$ het hoekverschil tussen spanning en stroom.
    ...
    Tel de twee aflezingen op:

    $$P_{\text{totaal}} = P_1 + P_2 = 1681.9 + 962.1 = \boxed{\mathbf{2644\text{ W}}}$$

    Dat klopt met het resultaat uit vraag 2 (2643.9 W). Het kleine verschil komt door afronding.
